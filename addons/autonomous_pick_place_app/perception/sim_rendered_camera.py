"""Drop-in RealSense replacement reading Genesis sim camera relay."""
...
from perception.realsense_camera import RealSenseFrame

from engine.sim_camera.subscriber import SimCameraSubscriber


class SimRenderedCamera:
    """RealSenseCamera-compatible adapter over sim→ctrl ZMQ relay."""
    ...
    def capture(self, *, retries: int = 40, retry_sleep_s: float = 0.05) -> RealSenseFrame:
        ...
        for _ in range(attempts):
            ...
            last_exc = RuntimeError("no sim camera frame received")
            if retry_sleep_s > 0:
                import time

                time.sleep(float(retry_sleep_s))
        raise last_exc or RuntimeError("no sim camera frame received")
