...
class AuthorityGeneration:
    ...
    def _write_bundle(
        ...
        role_views = root / "roles"
        role_views.mkdir(mode=0o700)
        for identity in selected:
            role_keystore = role_views / identity.role / "keystore"
            role_keystore.mkdir(parents=True, mode=0o700)
            copy_secure_tree(
                self.keystore / "public",
                role_keystore / "public",
            )
            copy_secure_tree(
