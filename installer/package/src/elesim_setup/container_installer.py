...
def _viewer_xhost_function(
    ...
    *,
    ...
    return (
        ...
        "  local best_display=\"\" best_authority=\"\" best_rank=99 best_count=0 best_candidates=\"\"\n"
        ...
        "    [[ $socket_uid == \"$viewer_xhost_runtime_uid\" ]] && return 0\n"
        ...
        "      [[ $authority_uid == \"$viewer_xhost_runtime_uid\" ]] || continue\n"
        ...
        "  viewer_add_authority_candidate \"/run/user/$viewer_xhost_runtime_uid/gdm/Xauthority\"\n"
        ...
        "      best_count=1\n"
        "      best_candidates=$selected_display\n"
        "    elif (( selected_rank == best_rank )); then\n"
        "      [[ $selected_display == \"$best_display\" ]] && continue\n"
        "      best_count=$((best_count + 1))\n"
        "      best_candidates=\"$best_candidates, $selected_display\"\n"
        "    fi\n"
        "  done\n"
        "  if [[ -z $best_display ]]; then\n"
        "    printf '현재 실행 사용자(uid=%s)가 소유한 접속 가능한 X11 세션을 찾지 못했습니다.\\n' \"$viewer_xhost_runtime_uid\" >&2\n"
        "    return 64\n"
        "  fi\n"
        "  if (( best_count > 1 )); then\n"
        "    printf '현재 실행 사용자의 물리/활성 X11 세션이 여러 개라 Viewer DISPLAY를 결정할 수 없습니다: %s\\n' \"$best_candidates\" >&2\n"
        "    return 64\n"
        "  fi\n"
