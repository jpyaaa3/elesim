"""Protocol-v3 adapter for the Genesis runtime's local state sockets."""
...
from elesim_protocol import EndpointClient
...
class SimProtocolBridge:
    def __init__(
        ...
        *,
        ...
        self.current_q = default_start_sim_q(self.mapping)
        self.state_meta: dict[str, Any] = {}

    def start(self) -> None:
        self.thread = threading.Thread(target=self._run, name="simulator-v3-bridge", daemon=True)
        self.thread.start()
                    ...
                    while True:
                        ...
                        if self.controller_id:
                            client.send(
                                ...
                                payload={
                                    **pack_state(u=sim_q_to_control_u(self.current_q, self.mapping), q=self.current_q),
    ...
    def _apply_command(self, payload: dict[str, Any]) -> tuple[bool, str]:
        ...
        if command == "target":
            ...
            if isinstance(raw_q, dict):
                ...
                self.current_q = type(self.current_q)(*(float(value) for value in raw_q))
    ...
    def _publish_state(self, socket: zmq.Socket) -> None:
        message = pack_state(
            u=sim_q_to_control_u(self.current_q, self.mapping),
            q=self.current_q,
            **self._pack_state_kwargs(),
        )
