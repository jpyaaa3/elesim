"""Runtime validation for a prebuilt simulator model bundle."""
...
SCHEMA_VERSION = 1
BUNDLE_TYPE = "elesim.simulator-model"
...
class ModelBundleError(ValueError):
    """The simulator model bundle is unsafe, incomplete or corrupted."""
...
def _validate_header(metadata: dict[str, Any]) -> Mapping[str, object]:
    ...
    if metadata.get("entrypoints") != ENTRYPOINTS:
        raise ModelBundleError("bundle entrypoints do not match the simulator contract")
...
def resolve_model_bundle(
    ...
    for root in _discovery_roots():
        candidate = root / "model/bundles/default"
        if (candidate / METADATA_NAME).is_file():
            return _checked_bundle_path(candidate)

    raise ModelBundleError(
        "default model bundle not found; pass --model-bundle or set "
        f"{MODEL_BUNDLE_ENV}"
    )
