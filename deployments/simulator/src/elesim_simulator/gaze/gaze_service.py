...
from elesim_simulator.pick.control_ownership import (
    ControlOwner,
    ControlOwnership,
    ControlOwnershipError,
    ControlState,
)
from elesim_simulator.gaze.stabilizer import GazeStabilizer, GazeStabilizerConfig, resolve_walking_gaze_mode
from elesim_simulator.gaze.preview_lite import PitchLeadEstimator, resolve_pitch_rate
from elesim_simulator.gaze.preview_mpc import solve_preview_du
from elesim_simulator.observability.walking_metrics import CameraMetricsLogger, _env_run_id
from elesim_simulator.observability.tracing import traced_thread_target
from elesim_simulator.vision.visual_servoing.uv_jacobian import default_uv_jacobian

if TYPE_CHECKING:
    from elesim_simulator.pick.actions import ControlService
...
class GazeControlService:
    ...
    def start_stop_and_grasp_demo(self) -> None:
        ...
        self._demo_thread = threading.Thread(
            target=traced_thread_target("gaze.stop_and_grasp_demo", _demo),
            name="gaze-demo4",
            daemon=True,
        )
    ...
    def _start(
        ...
        *,
        ...
        self._worker = threading.Thread(
            target=traced_thread_target("gaze.worker", self._worker_loop, mode=mode, gaze_mode=gaze_mode),
            name=f"gaze-{mode}",
            daemon=True,
        )
        self._worker.start()
