...
if TYPE_CHECKING:
    from elesim_simulator.config import PerceptionConfig
...
def _bbox_tracker_from_config(cfg: "PerceptionConfig") -> Any:
    from elesim_simulator.vision.perception.visual_tracker import BboxTracker, CsrtTrackerTuning  # type: ignore[import-not-found]
...
class PerceptionCapture:
    ...
    def _run(self) -> None:
        ...
        try:
            from elesim_simulator.vision.perception.pipeline import (
            ...
            from elesim_simulator.vision.perception.detector import create_detector, load_detector_config
            from elesim_simulator.vision.perception.preview import (
                close_preview,
                draw_detection_overlay,
                show_preview as render_preview_frame,
            )
            from elesim_simulator.vision.perception.realsense_camera import RealSenseCamera, RealSenseUnavailableError
            from elesim_simulator.vision.perception.visual_tracker import BboxTracker, detection_from_bbox
            from elesim_simulator.vision.perception.yolo_detector import YoloUnavailableError
        except Exception as exc:
        ...
        try:
            if mode == "mock":
                ...
                from elesim_simulator.vision.perception.sim_rendered_camera import SimRenderedCamera
    ...
    def _refine_detection_for_publish(
        ...
        *,
        ...
        if erode_px <= 0:
            return det
        _ensure_pick_place_path()
        from elesim_simulator.vision.perception.detection_utils import refine_detection_mask  # type: ignore[import-not-found]
    ...
    def _world_from_sim_frame(frame: Any, p_camera: np.ndarray) -> Optional[tuple[float, float, float]]:
        ...
        if origin is None or look is None or right is None:
            return None
        try:
            from elesim_simulator.vision.sim_camera.pose import camera_point_to_world_from_axes
        ...
        if mock_world is not None:
            ...
            if sim_world is not None:
                ...
                print(
                    ...
                    "restart elesim-simulator after update (world coords will use endpoint FK until then)"
                )
    ...
    def _run_camera_search_track(self, **kwargs: Any) -> None:
        ...
        _ensure_pick_place_path()
        from elesim_simulator.vision.perception.detection_utils import bbox_xyxy_area, detection_init_bbox  # type: ignore[import-not-found]
    ...
    def _run_camera_yolo_seg(self, **kwargs: Any) -> None:
        ...
        _ensure_pick_place_path()
        from elesim_simulator.vision.perception.detection_utils import (  # type: ignore[import-not-found]
            bbox_xyxy_area,
            detection_center_pixel,
            detection_init_bbox,
            detection_mask_coast_aligned,
        )
        from elesim_simulator.vision.perception.visual_tracker import detection_from_bbox  # type: ignore[import-not-found]
    ...
    def _run_mock_search_track(self, **kwargs: Any) -> None:
        ...
        _ensure_pick_place_path()
        from elesim_simulator.vision.perception.detection_utils import detection_init_bbox  # type: ignore[import-not-found]
