"""RealSense D435i color and aligned depth capture."""
...
from elesim_simulator.vision.perception.depth_pose import CameraIntrinsics
...
class RealSenseCamera:
    def __init__(
        ...
        *,
        ...
        self._config.enable_stream(rs.stream.color, int(color_width), int(color_height), rs.format.bgr8, int(fps))
        self._config.enable_stream(rs.stream.depth, int(color_width), int(color_height), rs.format.z16, int(fps))
    ...
    def start(self) -> None:
        try:
            ...
            raise RealSenseUnavailableError(f"failed to start RealSense pipeline: {exc}") from exc
        self._color_options_locked = False
    ...
    def capture(self, *, timeout_ms: int = 5000) -> RealSenseFrame:
        ...
        if not self._color_options_locked:
            self._lock_color_options()
        try:
            frames = self._pipeline.wait_for_frames(max(1, int(timeout_ms)))
        except TypeError:
            frames = self._pipeline.wait_for_frames()
        ...
        return RealSenseFrame(color_bgr=color, depth_raw=depth, depth_scale=float(self._depth_scale), intrinsics=intrinsics)
