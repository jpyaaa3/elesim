...
def load_solver_context(config_path: str) -> tuple[AppConfigBundle, dict[str, Any]]:
    ...
    raise FileNotFoundError(
        f"generated arm model not found: {model_path}; run tooling/model_builder/build_arm_model.py"
    )

    # Development-only source assembly path retained below for model-builder history.
    ...
    if bool(bundle.sim_config.rebuild_assembly) or (not os.path.isfile(manifest_path)):
        ...
        raise RuntimeError("controller deployments cannot rebuild assembly assets")
