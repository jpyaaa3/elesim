...
- 상태: open.
- 맥락: `config.ini`는 localhost endpoint를 쓰고, `config.pc.ini`와 `config.jetson.ini`는 명시적인 PC/Jetson TCP endpoint를 가진다.
...
- 제약: 현재 repo에는 local GO2 assets (`assets/go2/`)와 `builder/go2_arm_merger.py`가 추가되어 있으므로 current repo를 무작정 덮어쓰면 안 된다.
- 통합된 것:
  ...
  - `config.pc.ini`, `config.jetson.ini`, `perception_worker.py`, update의 GO2 hardware tests를 추가했다.
- 검증:
