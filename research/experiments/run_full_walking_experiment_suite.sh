...
_run() {
  echo ""
  echo "========== $* =========="
  python research/experiments/run_walking_baseline_batch.py "${COMMON[@]}" "$@"
}
...

{
  ...
  for rid in "${RUNS[@]}"; do
    python research/analysis/analyze_walking_metrics.py "$rid" --log-dir "$LOG_DIR" --merged "${ANALYZE_ARGS[@]}" >/dev/null || true
  done

  echo "=== 4-way compare (trial 005 representative; full trial) ==="
  python research/analysis/analyze_walking_metrics.py --log-dir "$LOG_DIR" "${ANALYZE_ARGS[@]}" --compare \
  ...
  python research/analysis/make_gait_preview_final_figures.py \
    --log-dir "$LOG_DIR" \
    --compare "$LOG_DIR/compare_summary.json" \
    --notes-filter "$NOTES_TAG" \
    "${FIG_ARGS[@]}" \
    --out research/results/pitch_preview_final
