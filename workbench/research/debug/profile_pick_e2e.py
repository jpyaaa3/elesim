...
Full stack (requires elesim-sim; this tool owns Pilot on the DDS graph):
  ELESIM_PROFILE_PICK=1 python workbench/research/debug/profile_pick_e2e.py --phases look,ready

Compute-only (no host):
  ELESIM_PROFILE_PICK=1 python workbench/research/debug/profile_pick_e2e.py --compute-only --cprofile
"""
...
from workbench.research.support.pilot_runtime import start_tool_pilot
    ...
    *,
    ...
    try:
        ...
        if host_state is None or not host_state.connected:
            print(
                "[profile] target not connected - start sim and pilot "
