...

{
  ...
  for rid in "${RUNS[@]}"; do
    python workbench/research/analysis/analyze_walking_metrics.py "$rid" --log-dir "$LOG_DIR" --merged >/dev/null || true
  done

  echo "=== 4-way compare ==="
  python workbench/research/analysis/analyze_walking_metrics.py --log-dir "$LOG_DIR" --compare \
