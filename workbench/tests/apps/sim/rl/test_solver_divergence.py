...
def test_an_unrelated_error_is_not_masked_by_a_stale_solver_mask(monkeypatch):
    solver = _Solver(torch.tensor([True, False]))
    _patch_qd(monkeypatch, solver)
    env = _Env(2, solver, raises=1)
    env.scene.step = lambda: (_ for _ in ()).throw(RuntimeError("unrelated"))
...
@pytest.mark.parametrize(
    "mask",
    [torch.tensor([True]), torch.tensor([[True, False]]), torch.tensor([1, 0])],
)
def test_malformed_solver_mask_does_not_enable_recovery(monkeypatch, mask):
    solver = _Solver(mask)
    _patch_qd(monkeypatch, solver)
    env = _Env(2, solver, raises=1)
    with pytest.raises(RuntimeError, match="nan"):
