...
MODEL_ROOT = REPO_ROOT / "payload" / "data" / "models" / "assemblies" / "zed-mini"
CONFIG_ROOT = REPO_ROOT / "payload" / "config" / "sim"
CALIBRATION_ROOT = REPO_ROOT / "payload" / "data" / "calibration" / "cameras"
...
def test_hand_eye_worker_uses_the_arm_visual_tree() -> None:
    ...
    spec = CameraRenderSpec(
        ...
        floor=False,
        hand_eye_config=str(
            CALIBRATION_ROOT / "zed_mini.hand_eye.json"
        ),
...
def test_real_async_camera_workers_keep_hand_eye_and_observer_live() -> None:
    ...
    arm_urdf = root / "arm.urdf"
    robot_urdf = root / "robot.urdf"
    calibration = CALIBRATION_ROOT / "zed_mini.hand_eye.json"
