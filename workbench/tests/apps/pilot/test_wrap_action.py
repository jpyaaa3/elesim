...
def _require_export_for_policy_tests(request):
    policy_tests = {
        "test_a_step_that_never_settles_stops_the_attempt",
        "test_a_lift_asked_for_before_the_arm_is_curled_is_held_back",
        "test_the_gate_admits_the_curl_a_wrap_actually_reaches",
        "test_a_motor_over_the_load_limit_stops_the_attempt",
        "test_the_load_limit_can_be_disabled",
        "test_the_lift_holds_the_grasp_and_only_unwinds_roll",
        "test_motor_load_opens_the_lift_gate_on_its_own",
        "test_an_overload_is_not_turned_into_a_lift",
        "test_contact_inside_the_ramp_ends_the_step_where_it_happened",
        "test_the_curl_command_is_capped_short_of_the_joint_limit",
        "test_the_cap_can_be_disabled",
        "test_a_step_does_not_wait_for_the_arm_to_come_to_rest",
    }
    if request.node.name in policy_tests and not (DEPLOY / "policy.pt").is_file():
        pytest.skip("exported policy is unavailable")


def _cfg(**kw) -> WrapGraspConfig:
    if not (DEPLOY / "policy.pt").is_file():
        pytest.skip("deploy/policy.pt is missing (generated by elesim_sim.rl.export)")
...
def test_missing_files_say_where_they_were_looked_for(tmp_path):
    ...
    with _pytest.raises(FileNotFoundError, match="Searched:"):
        WrapGraspRunner(cfg, read_joints=lambda: (0, 0, 0, 0),
                        command_waypoint=lambda w, t: True)


@pytest.mark.skipif(not (DEPLOY / "policy.pt").is_file(), reason="exported policy is unavailable")
def test_bare_filenames_under_a_search_root_are_found(tmp_path):
    ...
    assert not runner.policy.iface.expects_load
...
def test_a_waypoint_the_arm_cannot_reach_stops_the_attempt():
    ...
    assert out.steps == 1 and "waypoint was not reached" in out.reason
...
def test_a_bad_joint_reading_is_refused():
    ...
    assert "expected 4 joint estimates" in out.reason
...
def test_an_attempt_runs_off_the_request_thread_and_reports_through_the_snapshot():
    ...
    assert service.wrap_grasp_running() is False
    assert service.wrap_grasp_result() == "Lift executed · 7 steps · roll 0° (retention unverified)"
...
def test_a_failed_attempt_surfaces_as_a_result_rather_than_killing_the_worker():
    ...
    service._wrap_worker.join(5.0)
    assert service.wrap_grasp_running() is False
    assert "policy file missing" in service.wrap_grasp_result()
...
def test_a_step_that_never_settles_stops_the_attempt():
    ...
    assert outcome.reason == "waypoint was not reached"
...
def test_a_lift_asked_for_before_the_arm_is_curled_is_held_back():
    ...
    assert "lift requests held" in __import__(
...
def test_a_motor_over_the_load_limit_stops_the_attempt():
    ...
    assert outcome.reason == "load limit"
...
def test_the_lift_holds_the_grasp_and_only_unwinds_roll():
    ...
    assert sent, "roll ramp did not run"
...
def test_motor_load_opens_the_lift_gate_on_its_own():
    ...
    assert outcome.lift_requests_held == 0
    assert lifted, "lift did not run"
