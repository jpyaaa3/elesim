...
from elesim_pilot.vision.perception.detector import HsvDetector, create_detector, load_detector_config
from elesim_pilot.vision.perception.pipeline import resolve_detector_cfg, run_mock_frame


class TestDetectorConfig(unittest.TestCase):
    def test_config_yaml_resolves_detector_preset_path(self) -> None:
        bundle = load_app_config(str(ROOT / "payload/config/pilot/config.yaml"), mode="pc")
    ...
    def test_sim_hsv_preset_uses_target_label(self) -> None:
        cfg = load_detector_config(
            ROOT / "payload/config/pilot/perception/detector.sim_hsv.json"
        )
    ...
    def test_real_green_hsv_preset_accepts_shadow_green_largest_blob(self) -> None:
        cfg = load_detector_config(
            ROOT / "payload/config/pilot/perception/detector.real_green_hsv.json"
        )
        detector = create_detector(cfg)
        self.assertIsInstance(detector, HsvDetector)
    ...
    def test_yolo_example_has_no_mock_fallback_fields(self) -> None:
        cfg = load_detector_config(
            ROOT / "payload/config/pilot/perception/detector.yolo.example.json"
        )
