...
def _minimal_snapshot_members(*, project: bytes = b"[project]\n") -> dict[str, bytes]:
    members: dict[str, bytes] = {
        ...
        "payload/runtime/common/elesim_interfaces/package.xml": b"",
        ...
        "payload/data/models/arm/default.json": b"{}\n",
    ...
    for role in ("pilot", "sim", "ui", "robot"):
        ...
        application = bootstrap_module._BOOTSTRAP_ROLE_APPLICATIONS[role]
        members[(application / "pyproject.toml").as_posix()] = b"[project]\n"
        members[(runtime / "requirements.lock").as_posix()] = b""
        members[(application / f"elesim_{role}/__init__.py").as_posix()] = b""
...
@pytest.mark.parametrize(
    "relative",
    (
        "payload/runtime/docker/pilot/app/elesim_pilot/main.py",
        "payload/runtime/native/robot/app/elesim_robot/go2/unitree_bridge_daemon.py",
        "payload/config/pilot/runtime.yaml",
        "payload/config/sim/runtime.yaml",
    ),
)
...
def test_source_snapshot_rejects_unowned_protocol_python_module(
    ...
    _write_valid_snapshot(snapshot)
    root = snapshot / "elesim-main"
    (root / "payload/runtime/common/protocol/elesim_protocol/dummy.py").write_text(
        "", encoding="utf-8"
    )
...
def test_protocol_tracing_module_is_owned_by_bootstrap_manifest() -> None:
    assert (
        PurePosixPath("payload/runtime/common/protocol/elesim_protocol/tracing.py")
...
def test_protocol_encoded_rgbd_module_is_owned_by_bootstrap_manifest() -> None:
    assert (
        PurePosixPath("payload/runtime/common/protocol/elesim_protocol/encoded_rgbd.py")
        in bootstrap_module._BOOTSTRAP_PROTOCOL_PYTHON_FILES
    )
...
def test_sim_mock_object_is_owned_by_bootstrap_manifest() -> None:
    assert (
        ...
        in bootstrap_module._BOOTSTRAP_REQUIRED_TREE_FILES
    )
...
def test_source_snapshot_requires_every_cmake_declared_rosidl_source(
    ...
    _write_valid_snapshot(snapshot)
    root = snapshot / "elesim-main"
    interfaces = root / "payload/runtime/common/elesim_interfaces"
...
def test_download_source_caches_only_install_source_boundary(
    ...
    payload = _archive_payload(
        ...
        extra_members={
            ...
            "payload/config/pilot/perception/detector.yolo.example.json": b"{}\n",
            "payload/config/pilot/runtime.public.example.yaml": b"public: true\n",
            "payload/config/sim/runtime.public.example.yaml": b"public: true\n",
            "payload/config/ui/public.example.yaml": b"public: true\n",
            "payload/config/robot/public.example.yaml": b"public: true\n",
            "payload/data/models/assemblies/zed-mini/bundle.json": b"{}\n",
            "payload/data/models/assemblies/d435/bundle.json": b"{}\n",
            "workbench/tests/apps/pilot/test_dummy.py": b"raise AssertionError\n",
            "workbench/tests/setup/test_dummy.py": b"raise AssertionError\n",
            "workbench/research/dummy.bin": b"research-only\n",
            "workbench/tools/setup_preview.py": b"raise AssertionError\n",
            "docs/README.md": b"repository-only docs\n",
            "payload/runtime/docker/sim/app/elesim_sim/rl/train.py": b"raise AssertionError\n",
        },
    )
    monkeypatch.setattr(
        urllib.request,
    ...
    assert (root / "payload/runtime/docker/pilot/app/elesim_pilot/main.py").is_file()
    assert (root / "payload/config/pilot/perception/detector.yolo.example.json").is_file()
    assert not (root / "payload/config/pilot/runtime.public.example.yaml").exists()
    assert not (root / "payload/config/sim/runtime.public.example.yaml").exists()
    assert not (root / "payload/config/ui/public.example.yaml").exists()
    assert not (root / "payload/config/robot/public.example.yaml").exists()
    assert (root / "payload/data/models/assemblies/zed-mini/bundle.json").is_file()
    assert (root / "payload/data/models/assemblies/d435/bundle.json").is_file()
    assert not (root / "workbench/tests/apps/pilot").exists()
    assert not (root / "workbench/tests/setup").exists()
    assert not (root / "workbench/research").exists()
    assert not (root / "workbench/tools/setup_preview.py").exists()
    assert not (root / "docs").exists()
    assert not (root / "payload/runtime/docker/sim/app/elesim_sim/rl").exists()
...
def test_download_source_replaces_legacy_snapshot_outside_install_boundary(
    ...
    url = "https://archives.example/elesim.tar.gz"
    first = download_source(url, tmp_path)
    source_only = first / "workbench/research/source-only.txt"
    ...
    assert second == first
    assert not (second / "workbench").exists()
