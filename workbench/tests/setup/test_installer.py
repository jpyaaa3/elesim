...
from elesim_setup.configuration import app_directory
...
def test_robot_lock_covers_system_pynacl_dependency() -> None:
    requirements = (
        Path(__file__).resolve().parents[3] / "payload/runtime/native/robot/requirements.lock"
    ).read_text(encoding="utf-8")
...
def test_native_install_records_host_uninstaller_and_exact_systemd_hashes(
    ...
    monkeypatch.setattr(
        installer,
        "_install_role",
        lambda role: (state.prefix_path / "apps" / role).mkdir(parents=True),
    )
    monkeypatch.setattr(
        installer_module,
    ...
    for name, ownership in units.items():
        generated = state.prefix_path / "apps/robot/systemd" / name
