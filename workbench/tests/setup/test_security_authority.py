...
def test_host_bundle_contains_public_and_only_assigned_enclaves(
    ...
    assert (
        exported.path
        / "apps/pilot/keystore/public/identity_ca.cert.pem"
    ).is_file()
    assert not (
        exported.path
        / "keystore/enclaves"
    ...
    assert not (exported.path / "apps/pilot/keystore/private").exists()
