...
Genesis, DDS or WebRTC, and it keeps a NumPy fallback for CPU-only installs and
for unusual render outputs.  A timing sink is intentionally tiny so the
runtime can account for conversion stages without coupling this module to its
perf logger.
"""
    ...
    *,
    ...
    if prefer_gpu and _is_cuda_tensor(value):
        try:
            ...
            # A device-specific conversion failure must not take down Sim.  The
            # host fallback below preserves the previous behavior.
            pass
