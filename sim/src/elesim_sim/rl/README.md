...
python -m pip install -r sim/requirements.lock
python -m pip install rsl-rl-lib tensorboard "numpy<2"
python -m pip install --no-deps -e packages/protocol -e sim
```
...
Runs land in `sim/rl_runs/<experiment_name>/stage<N>_<stamp>/`. **Reusing a
...
```bash
tensorboard --logdir sim/rl_runs/wrap_grasp --port 6006
...
```bash
python -m elesim_sim.rl.train --resume sim/rl_runs/wrap_grasp/stage1_run1/model_200.pt --stamp run2
```
...
spawn height and the UI pose presets from `sim/config/config.yaml`. Config
