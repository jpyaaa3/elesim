...
def test_v4_envelope_round_trip() -> None:
    ...
    assert decoded.version == PROTOCOL_VERSION == 4
...
def test_media_stream_descriptor_rejects_invalid_security_combinations() -> None:
    with pytest.raises(ProtocolError, match="CURVE server key"):
        MediaStreamDescriptor(
            transport="zmq",
            media_kind="rgbd",
            endpoint="tcp://10.0.0.2:5568",
            security="curve",
        )
        ...
        EndpointDescriptor.from_dict(
            {
                ...
                "role": "simulator",
                "capabilities": [],
                "streams": {"rgbd": "tcp://127.0.0.1:5568"},
                "instance_id": "sim-instance",
            }
        )


def test_protocol_v4_has_no_legacy_camera_input_message() -> None:
