...
def test_v5_envelope_round_trip() -> None:
    ...
    assert decoded.version == PROTOCOL_VERSION == 5
...
def test_v4_is_rejected() -> None:
    message = make_envelope("discover", "robot-a", seq=1).to_dict()
    message["version"] = 4
    with pytest.raises(ProtocolError, match="expected 5"):
...
def test_protocol_v5_has_no_legacy_camera_input_message() -> None:
