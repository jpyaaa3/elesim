...
from apps.robot_agent.main import RobotRuntime
from engine.core.protocol import EndpointDescriptor, SimMappingConfig, make_envelope
...
def test_target_is_converted_and_sent_to_hardware() -> None:
    ...
    ok, reason = runtime.apply(
        _command(
            ...
            {
                "command": "target",
                "q": {"linear_m": -0.1, "roll_rad": 0.1, "theta1_rad": 0.2, "theta2_rad": -0.2},
            },
        )
    )
...
def test_stale_sequence_and_wrong_lease_are_rejected() -> None:
    ...
    wrong = make_envelope(
        "command", "controller-a", target_id="robot-a", payload={"command": "target"}, seq=3, lease_id="wrong"
    )
