...
class RunContextTests(unittest.TestCase):
    ...
    def test_with_preview_stats_meta(self) -> None:
        ctx = RunContext.from_cli(run_id="exp_p", gaze_mode="preview")
        ...
        ctx = ctx.with_preview_stats(
            ...
            gait_template_path="logs/gait_templates/t.json",
        )
        with tempfile.TemporaryDirectory() as td:
            path = ctx.write_meta(td)
            payload = json.loads(path.read_text(encoding="utf-8"))
            self.assertEqual(payload["requested_gaze_mode"], "preview")
            self.assertEqual(payload["actual_gaze_mode"], "preview")
            self.assertAlmostEqual(payload["preview_used_ratio"], 0.8)
            self.assertAlmostEqual(payload["preview_fallback_ratio"], 0.2)
            self.assertEqual(payload["preview_type"], "gait_phase")

    def test_actual_gaze_mode_uv_when_low_preview_ratio(self) -> None:
        ctx = RunContext.from_cli(run_id="exp_p2", gaze_mode="preview")
