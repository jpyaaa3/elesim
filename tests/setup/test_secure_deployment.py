...
def _bundle(host_id: str, generation: str = "g2") -> SecurityBundle:
    ...
    for role in roles:
        files.extend(
            (
                SecurityFile(
                    f"apps/{role}/keystore/public/identity_ca.cert.pem",
                    b"public",
                    0o644,
                ),
                SecurityFile(
                    f"apps/{role}/keystore/enclaves/{role}/key.pem",
...
def test_bundle_manifest_is_bounded_hashed_and_contains_no_payload() -> None:
    ...
    private_file = files["enclaves/server/key.pem"]
    assert private_file["mode"] == "0600"
...
def test_role_scoped_bundle_keeps_only_selected_role_material() -> None:
    bundle = SecurityBundle(
        ...
        (
            ...
            SecurityFile("apps/pilot/keystore/enclaves/elesim/lab/pilot/main/key.pem", b"pilot"),
            SecurityFile("apps/robot/keystore/enclaves/elesim/lab/robot/main/key.pem", b"robot"),
        ),
    ).validate()
    ...
    assert [item.relative_path for item in robot_view.files] == [
        ...
        "apps/robot/keystore/enclaves/elesim/lab/robot/main/key.pem",
    ]
    ...
    assert [item.relative_path for item in pilot_view.files] == [
        ...
        "apps/pilot/keystore/enclaves/elesim/lab/pilot/main/key.pem",
    ]
...
def test_local_host_operations_use_install_root_security_directory(
    ...
    pilot_root = Path(host.install_root) / "security/apps/pilot"
    ui_root = Path(host.install_root) / "security/apps/ui"
...
def test_role_view_switch_and_clear_keep_stable_root_inode(tmp_path: Path) -> None:
    ...
    topology = ConnectionTopology.from_dict(raw)
    host = topology.host("laptop")
    role_root = Path(host.install_root) / "security/apps/pilot"
...
def test_role_view_activation_refuses_symlink_root(tmp_path: Path) -> None:
    ...
    (security / "apps").mkdir(parents=True)
    (security / "apps/pilot").symlink_to(target, target_is_directory=True)
    operations = LocalHostOperations(FakeLifecycle(), topology)
    operations.stage(host, _bundle("laptop"))
