    ...
    payload.update(
        {
            ...
            "developer_workspace": str(tmp_path / "source"),
        }
    )
    request = SetupRequest.from_dict(payload)

    state = request.validate(_capabilities()).to_install_state()

    assert state.roles == ("sim", "pilot", "ui")
    assert state.developer_attachment.enabled is True
    assert state.developer_attachment.workspace_path == (tmp_path / "source").resolve()
    assert state.runtime_text_logs.enabled is True
...
def test_legacy_developer_edition_is_rejected_with_migration_guidance(
    tmp_path: Path,
) -> None:
    payload = _payload(tmp_path)
    payload["edition"] = "developer"

    with pytest.raises(ValueError, match="developer attachment"):
        SetupRequest.from_dict(payload)


def test_developer_wslg_rejects_string_booleans(tmp_path: Path) -> None:
    payload = _payload(tmp_path)
    payload.update(
        {
            "developer_attachment": True,
            "developer_workspace": str(tmp_path / "source"),
            "developer_wslg": "false",
        }
    )

    with pytest.raises(ValueError, match="developer_wslg"):
        SetupRequest.from_dict(payload)


def test_developer_wslg_requires_detected_wslg_host(tmp_path: Path) -> None:
    payload = _payload(tmp_path)
    payload.update(
        {
            "developer_attachment": True,
    ...
    with pytest.raises(ValueError, match="WSLg host"):
        request.validate(_capabilities())
    request.validate(_capabilities(wslg=True))
