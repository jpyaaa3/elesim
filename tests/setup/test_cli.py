...
def test_generated_dds_xml_must_match_state(local_state) -> None:
    ...
    ContainerInstaller(state).run()
    xml_path = state.prefix_path / "apps/pilot/config/cyclonedds.xml"
    xml_path.write_text(
...
def test_generated_role_enclave_and_key_must_match_state(local_state) -> None:
    ...
    role_keystore = initial.prefix_path / "security/apps/sim"
    state = replace(
        initial,
        dds=DdsSettings(
...
def test_network_configure_rewrites_all_installed_role_configs(local_state, tmp_path: Path) -> None:
    ...
    assert (state.prefix_path / "apps/pilot/config/runtime.installed.yaml").is_file()
    assert (state.prefix_path / "apps/ui/config/installed.yaml").is_file()
