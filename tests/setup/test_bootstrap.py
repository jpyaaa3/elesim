...
def test_download_source_caches_only_install_source_boundary(
    ...
    payload = _archive_payload(
        ...
        extra_members={
            ...
            "payload/data/models/assemblies/zed-mini/bundle.json": b"{}\n",
            "payload/data/models/assemblies/d435/bundle.json": b"{}\n",
            "tests/apps/pilot/test_dummy.py": b"raise AssertionError\n",
            "tests/setup/test_dummy.py": b"raise AssertionError\n",
    ...
    assert not (root / "tests/apps/pilot").exists()
    assert not (root / "tests/setup").exists()
...
def test_failed_shell_download_preserves_previous_bootstrap(tmp_path: Path) -> None:
    script = Path(__file__).resolve().parents[2] / "installer/bootstrap/install.sh"
...
def test_bootstrap_reports_selected_docker_backend_and_tailscale_interfaces() -> None:
    script = (Path(__file__).resolve().parents[2] / "installer/bootstrap/install.sh").read_text(
        encoding="utf-8"
    )
...
def test_shell_forwards_custom_archive_without_exposing_it_in_docker_argv(
    ...
    script = Path(__file__).resolve().parents[2] / "installer/bootstrap/install.sh"
    bootstrap_source = Path(__file__).resolve().parents[2] / "installer/bootstrap/bootstrap.py"
    fake_bin = tmp_path / "bin"
    fake_bin.mkdir()
    curl = fake_bin / "curl"
    curl.write_text(
...
def test_bootstrap_venv_pins_ros_build_python_metadata_dependencies() -> None:
    script = Path(__file__).resolve().parents[2] / "installer/bootstrap/bootstrap.py"
