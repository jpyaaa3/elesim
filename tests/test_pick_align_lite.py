...
class TestAlignSkip(unittest.TestCase):
    def test_skips_refine_when_direction_already_good(self) -> None:
        ...
        with patch("engine.ik.ik_solver.solve_ik") as solve_ik, patch(
            "engine.ik.ik_kin._forward_grasp_direction_world",
            return_value=direction,
        ), patch("engine.ik._refine_orientation") as refine:
    ...
    def test_refines_when_direction_exceeds_skip_threshold(self) -> None:
        ...
        ), patch("engine.ik.ik_kin._forward_grasp_world", return_value=target), patch(
            "engine.ik._refine_orientation",
...
class TestLiteAlignMode(unittest.TestCase):
    def test_refine_orientation_uses_lite_path(self) -> None:
        ...
        with patch(
            "engine.ik.ik_aligner.refine_direction_lite",
            return_value=lite_result,
        ) as lite_fn, patch(
            "engine.ik.ik_aligner.refine_direction_with_position_hold",
        ) as full_fn:
