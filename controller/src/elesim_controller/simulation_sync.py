"""Synchronize long-running visual workflows with authoritative simulator state."""
...
class SimulationWorkflowSync:
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._run,
            name="controller-simulation-sync",
            daemon=True,
        )
        self._thread.start()
    ...
    def process_one(self, *, timeout_s: float = 0.0) -> bool:
        ...
        try:
            ...
            print(f"[control_agent] visual workflows stopped: {reason}")
        finally:
            with self._lock:
                self._pending = False
        return True
