"""Estimate object 3D position in the camera optical frame from depth and mask."""
...
def estimate_object_position_camera(
    ...
    *,
    ...
    """Return [x, y, z] in camera optical frame meters."""
    if mask.ndim != 2:
    ...
    x_m = (xs_i.astype(np.float64) - cx) * z_i / fx
    y_m = (ys_i.astype(np.float64) - cy) * z_i / fy
    p = np.array([float(np.median(x_m)), float(np.median(y_m)), float(np.median(z_i))], dtype=float)
