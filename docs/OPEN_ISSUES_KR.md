...
> `model/source/assets`, `model/bundles/default`이다. 과거 증거를 보존하기
...
- 최종 topology에는 Router, ZMQ, CurveZMQ, CURVE key, ZAP policy가 없다.
  Robot/Sim가 자기 motion lease를 소유하고 Sim가 별도 UI session을
...
- 상태: open.
- 기존 테스트는 observer/hand-eye media 독립성, Sim main-thread
...
- Managed Coturn은 REST HMAC secret을 Coturn과 같은 host의 Sim에만
  mount하고, Sim가 session-bound 단기 credential을 발급한다. UI에는
...
- 상태: open.
- 현재 deployments와 tools 전체에 broad exception handler가 약 267개 있다.
...
- Pilot/Sim는 direct protocol-v6 endpoint를 사용한다. sibling role의
...
### 역사적 기록: 2026-07-21 remote-sim refactor에서 닫힌 항목
...
- UI는 Pilot를 camera-input relay로 쓰지 않고 독립 Sim session을
  소유한다.
- Sim는 observer/hand-eye WebRTC view를 분리해 보내고 orbit, pan, zoom,
