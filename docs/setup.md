...
managed security view, logs, operator Authority를 제거하는 host-local factory
reset 경계다. Docker image와 BuildKit cache는 재사용 가능한 cache이므로
삭제하지 않는다. 다른 프로젝트의 home, source checkout, Tailscale admin
record, 외부 keystore/TURN credential은 EleSim이 생성했다는 소유 증거가 없으므로
보존한다. 외부 보안 상태의 폐기와 재발급은 `elesim-connections`의 managed
generation transaction에서 수행한다.
