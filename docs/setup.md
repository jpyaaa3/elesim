...
| WSL에서 `docker-credential-*.exe: exec format error` | `~/.docker/config.json`의 Windows credential helper와 Linux Docker CLI 조합 | bootstrap과 EleSim 이미지 build/update/release/dev/connection-manager 및 Tailscale sidecar pull wrapper가 깨진 `.exe` helper만 제거한 일회성 Docker config를 사용한다. 사용자 Docker credentials는 변경하지 않는다. 직접 `docker compose build`를 실행할 때는 Linux용 `DOCKER_CONFIG`를 지정한다. |
| `elesim connections`의 접속 URL/토큰이 좁은 터미널에서 잘림 | compact build progress의 한 줄 미리보기 | 최신 wrapper는 `[connection-manager]` 접속 줄 전체를 미리보기 축약 없이 출력하므로 터미널이 좁아도 줄바꿈해 확인할 수 있다. |
| update 후 `image is referenced in multiple repositories` cleanup 오류 | 같은 install-owned image ID에 여러 readable/legacy tag가 남아 있음 | update/start 자체는 rollback하지 않는다. 최신 wrapper는 ownership 검증을 통과한 tag를 개별적으로 제거하며, `docker image rm -f`나 전역 prune은 사용하지 않는다. |
| `elesim-uninstall`의 `No such image` 오류 | 다른 정리 작업이 해당 install-owned image를 먼저 제거했는지 | 최신 host uninstaller는 소유권 검증 후 발생한 정확한 Docker `No such image`만 이미 완료된 삭제로 처리하고 계속한다. 그 외 image 삭제 오류는 계속 실패한다. |
