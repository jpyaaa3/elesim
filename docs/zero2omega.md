...
8. 보안 profile이 `sros2`이면 첫 배포에서는 `보안 자료 생성·초기 배포`, 기존
   세대가 있으면 `키 재발급`을 누른다.
9. `전체 시작`을 누른다. 또는 각 host에서 `elesim-up`을 실행한다.

정상 흐름은 저장 후 2단계 보안 작업 하나를 선택하고 시작하는 흐름이다. 오류가 나면 `Abort`로 현재 작업을 안전한 경계에서
...
새로 `보안 자료 생성·초기 배포`를 반복하지 않는다. 기존 세대를 교체할 때만
`키 재발급`을 누른다. 이 작업은 모든 host를 사전 점검하고 새 세대를
원자적으로 활성화한다. 한 host가 실패하면 이전 세대로 되돌리는 것을 전제로
한다.
...
| 버튼 | 하는 일 | 하지 않는 일 |
| --- | --- | --- |
| 키 재발급 | 모든 host의 managed SROS2 generation 원자 교체 | 런타임 시작 또는 키 본문 입력 |
...
저장한 뒤 `보안 자료 생성·초기 배포`를 성공시킨다. 그 다음에 `elesim-up`을
실행한다.
...
서비스를 끌 때는 `elesim-down`을 사용한다. 연결관리자의 `전체 시작/정지`는
...
- [ ] `sros2`면 보안 자료 생성·초기 배포를 `elesim-up`보다 먼저 실행했다.
