...
8. 아직 저장하지 않고 주소 형식만 확인하려면 활성 COM을 **정확히 두 개**로
   맞추고 `두 호스트 점검`을 누른다. 이 버튼은 topology를 저장·배포하지
   않는다.
9. 보안 profile이 `sros2`이면 `보안 자료 생성·초기 배포`를 한 번 누른다.
...
`전체 보안 세대 교체`를 누른다. 이 작업은 모든 host를 사전 점검하고 새 세대를
