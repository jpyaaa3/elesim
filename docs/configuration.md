...
Pilot and sim preserve the existing YAML inheritance behavior:
...
or `runtime.installed.yaml`; Sim also receives `app.installed.yaml` for
...
regenerates every installed role from that state so Pilot, UI, Sim
and Robot cannot silently drift to incompatible system/domain IDs, RMW,
discovery, interface or security settings.

Sim runtime configuration also owns TURN credentials. Managed mode points
...
under `/run/secrets`, and never copies the external file into Pilot or UI.
...
hides CUDA and sets Sim `use_gpu` false.

## Model Inputs

Geometry and assembly settings are not runtime configuration. They are source
inputs under `model/source`, compiled by `model/builder`, and shipped
to the sim as `model/bundles/default`.
