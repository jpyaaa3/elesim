...
| Role | Configuration |
| --- | --- |
| Controller | `deployments/controller/config/` |
| Router | `deployments/router/config/` |
| Robot | `deployments/robot/config/` |
| Simulator | `deployments/simulator/config/` |
| UI | `deployments/ui/config/` |
...
are `deployments/controller/config/runtime.yaml` and
`deployments/simulator/config/runtime.yaml`. UI and robot use the equivalent
...
inputs under `model/source`, compiled by `tooling/model_builder`, and shipped
