...
def _load_connector_spec_from_static_frame(kind: PartKind, root_dir: Optional[str] = None) -> ConnectorSpec:
    frame_path = os.path.join(root_dir or DEFAULT_ASSET_ROOT_DIR, kind.value, f"{kind.value}_frame.json")
...
def _has_local_go2_urdf(root_dir: Optional[str] = None) -> bool:
    return os.path.isfile(os.path.join(root_dir or DEFAULT_ASSET_ROOT_DIR, "go2", "go2.urdf"))


def make_default_config(*, use_go2: bool = False, asset_root: Optional[str] = None) -> RobotBuildConfig:
    ...
    for kind in (
        ...
        connectors[kind] = _load_connector_spec_from_static_frame(kind, asset_root)
    kwargs = dict(
...
class AssetFinder:
    def __init__(self, root_dir: Optional[str] = None):
        self._root_dir = root_dir or DEFAULT_ASSET_ROOT_DIR
...
class ManifestWriter:
    ...
    def build(
        self,
        robot_graph: AssemblyPlan,
        layout: JointLayoutResult,
        out_dir: str,
        *,
        output_name: str = "blueprint.json",
    ) -> ManifestBuildResult:
        ...
        manifest_path = self._write_manifest(out_dir, manifest, output_name=output_name)
    ...
    def _write_manifest(
        ...
        *,
        ...
        manifest_path = os.path.join(out_dir, output_name)
...
def build_default_manifest(
    ...
    *,
    ...
    root_dir = asset_root or DEFAULT_ASSET_ROOT_DIR
    if bool(use_go2) and (not _has_local_go2_urdf(root_dir)):
        expected = os.path.join(root_dir, "go2", "go2.urdf")
    ...
    config = make_default_config(use_go2=False, asset_root=root_dir)
    asset_finder = AssetFinder(root_dir)
    ...
    result = ManifestWriter(config, policy, asset_finder=asset_finder).build(
        robot_graph,
        layout,
        output_dir,
