...
COMMON=(
  --config controller/config/default.yaml
  --motion forward
  --preset neutral
  --max-duration 30
...

{
  ...
  for rid in "${RUNS[@]}"; do
    python misc/tooling/analysis/analyze_walking_metrics.py "$rid" --log-dir "$LOG_DIR" --merged "${ANALYZE_ARGS[@]}" >/dev/null || true
  done

  echo "=== 4-way compare (trial 005 representative; full trial) ==="
  python misc/tooling/analysis/analyze_walking_metrics.py --log-dir "$LOG_DIR" "${ANALYZE_ARGS[@]}" --compare \
  ...
  python misc/tooling/analysis/make_gait_preview_final_figures.py \
    --log-dir "$LOG_DIR" \
    --compare "$LOG_DIR/compare_summary.json" \
    --notes-filter "$NOTES_TAG" \
    "${FIG_ARGS[@]}" \
    --out misc/results/pitch_preview_final
