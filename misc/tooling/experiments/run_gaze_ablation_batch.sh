...
COMMON=(--config controller/config/default.yaml --motion forward --preset neutral
...
python misc/tooling/experiments/run_walking_baseline_batch.py "${COMMON[@]}" \
...
for rid in "${RUNS[@]}"; do
  python misc/tooling/analysis/analyze_walking_metrics.py "$rid" --log-dir logs/walking_baseline --merged >/dev/null
done

echo "=== 3-way compare (trial 001) ==="
python misc/tooling/analysis/analyze_walking_metrics.py --log-dir logs/walking_baseline --compare \
...
python misc/tooling/analysis/analyze_walking_metrics.py --log-dir logs/walking_baseline --compare "${RUNS[@]}" \
