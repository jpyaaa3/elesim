...
ROLES = ("controller", "simulator", "ui", "robot")
SIMULATION_ROLES = ("controller", "simulator", "ui")
...
class DdsEndpoint:
    ...
    def from_dict(cls, raw: Mapping[str, Any]) -> "DdsEndpoint":
        values = _strict_object(raw, required={"address", "interface"}, name="dds")
        return cls(
...
class ManagedHost:
    ...
    def validate(self) -> "ManagedHost":
        ...
        if "robot" in roles:
            ...
            raise ValueError(
                "Controller, Simulator and UI hosts require container/Compose"
            )
    ...
    def from_dict(cls, raw: Mapping[str, Any]) -> "ConnectionTopology":
        ...
        if incoming_version == LEGACY_CONNECTION_SCHEMA_VERSION:
            ...
            raise ValueError(
                ...
                f"expected {LEGACY_CONNECTION_SCHEMA_VERSION} or {CONNECTION_SCHEMA_VERSION}"
            )
