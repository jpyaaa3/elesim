...
const slots = ["com1", "com2", "com3", "robot"];
const applicationRoles = ["controller", "simulator", "ui", "robot"];
const movableRoles = ["controller", "simulator", "ui"];
...
function visibleRoles() {
  return topologyMode === "simulation-only"
    ? ["controller", "simulator", "ui"] : applicationRoles;
}
...
function applyTopology(topology) {
  ...
  ["com1", "com2", "com3"].forEach((slot, index) => {
...
function setJobRunning(running) {
  ["save", "preflight", "provision", "deploy", "topology-mode"].forEach((id) => { byId(id).disabled = running; });
  byId("provision").disabled = running || byId("security").value !== "sros2";
  byId("rotate").disabled = running || byId("security").value !== "sros2";
...
async function initialize() {
  try {
    ...
    bindEvents();
    applyLanguage("ko");
    ["com1", "com2", "com3"].forEach(setCardActive);
