...
def _manifest(
    ...
    *,
    ...
    _write(security / "roles/controller/key.pem")
    _write(static / "generated.txt")
    _write(logs / "runs/one/controller.log")
...
def test_default_uninstall_preserves_logs_authority_external_and_foreign_files(
    ...
    assert logs.joinpath("runs/one/controller.log").is_file()
...
def test_nested_bind_mount_aborts_before_mutation(
    ...
    assert manifest.path.is_file()
    assert nested_mount.joinpath("controller/key.pem").is_file()
...
def test_docker_deletes_only_exact_manifest_objects(tmp_path: Path) -> None:
    ...
    docker = DockerOwnership(
        ...
        local_images=("elesim/simulator:local",),
    )
    ...
    assert ("docker", "image", "rm", "elesim/simulator:local") in runner.commands
