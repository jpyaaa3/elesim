    ...
    return ConnectionTopology(
        ...
        hosts=(
            ...
            ManagedHost(
                ...
                assignments=(RoleAssignment("simulator", "sim-main"),),
            ),
            ManagedHost(
                host_id="robot",
...
def test_simulation_only_topology_accepts_one_host_without_robot() -> None:
    ...
    assert restored == topology
    assert restored.hosts[0].roles == ("controller", "simulator", "ui")
...
def test_endpoint_ids_reject_canonical_dash_underscore_collisions() -> None:
    raw = _topology().to_dict()
    raw["hosts"][0]["assignments"][1]["endpoint_id"] = "controller_main"
...
@pytest.mark.parametrize("role", ["controller", "simulator", "ui", "robot"])
...
def test_duplicate_role_assignment_is_rejected() -> None:
    ...
    raw["hosts"][1]["assignments"].append(
        {"role": "controller", "endpoint_id": "controller-other"}
    )
