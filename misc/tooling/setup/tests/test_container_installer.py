...
def test_container_install_generates_ros_overlay_contexts_and_dds_environment(
    ...
    for role in state.roles:
        ...
        expected_container_names = {
            "controller": "elesim-pilot",
            "ui": "elesim-ui",
            "simulator": "elesim-sim",
        }
    ...
    role_wrapper = (state.bin_path / "elesim-simulator").read_text(
        encoding="utf-8"
    )
    assert "up -d --build --remove-orphans" in up_wrapper
    assert "down --remove-orphans" in down_wrapper
    assert "up --remove-orphans simulator" in role_wrapper
...
def test_static_discovery_is_exported_to_every_service(local_state) -> None:
    ...
    compose = _compose(state)
    for name in ("controller", "ui", "tools"):
...
def test_managed_coturn_is_owned_by_simulator_and_shares_only_turn_secret(
    ...
    assert (run / "simulator.log").read_text(encoding="utf-8") == (
        "saved log for simulator\n"
    )
...
def test_pending_managed_sros2_installs_coturn_but_refuses_application_start(
    ...
    assert compose["services"]["simulator"]["environment"][
        "ROS_SECURITY_KEYSTORE"
    ] == str(state.prefix_path / "security/roles/simulator")
...
def test_sros2_environment_and_keystore_mount_are_role_specific(
    ...
    keystore = tmp_path / "sros2"
    (keystore / "public").mkdir(parents=True)
    (keystore / "enclaves/prod/controller_main").mkdir(parents=True)
    ...
    (keystore / "enclaves/prod/controller_main/key.pem").write_text(
        "controller-key", encoding="utf-8"
    )
    (keystore / "enclaves/prod/ui_main/key.pem").write_text(
        "ui-key", encoding="utf-8"
    )
    for role in ("controller_main", "ui_main"):
    ...
    assert "ROS_SECURITY_ENCLAVE_OVERRIDE" not in services["controller"]["environment"]
    ...
    controller_view = state.prefix_path / "security/roles/controller"
    ui_view = state.prefix_path / "security/roles/ui"
    assert services["controller"]["environment"]["ROS_SECURITY_KEYSTORE"] == str(
        controller_view
    )
    assert f"{controller_view}:{controller_view}:ro" in services["controller"][
        "volumes"
    ]
    assert f"{ui_view}:{ui_view}:ro" not in services["controller"]["volumes"]
    assert not any(
        volume.startswith(f"{keystore}:")
        for volume in services["controller"]["volumes"]
    )
    assert (controller_view / "enclaves/prod/controller_main/key.pem").is_file()
    assert not (controller_view / "public/identity_ca.cert.pem").is_symlink()
    assert not (
        controller_view / "enclaves/prod/controller_main/governance.p7s"
    ).is_symlink()
    assert not (controller_view / "enclaves/prod/ui_main").exists()
    assert (ui_view / "enclaves/prod/ui_main/key.pem").is_file()
    assert not (ui_view / "enclaves/prod/controller_main").exists()
...
def test_runtime_log_failure_does_not_prevent_down_and_returns_nonzero(
    ...
    environment.update(
        {
            ...
            "ELESIM_FAIL_LOG_SERVICE": "controller",
    ...
    assert marker.is_file()
    assert "controller" in result.stderr
    run = next((state.prefix_path / "logs/runs").iterdir())
    assert (run / "controller.log").is_file()
