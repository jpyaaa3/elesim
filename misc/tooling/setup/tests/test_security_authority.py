...
def _issued_generation(
    ...
    controller = transaction.create_enclave("controller", "controller-main")
    ui = transaction.create_enclave("ui", "ui-main")
    simulator = transaction.create_enclave("simulator", "sim-west")
    return authority, transaction, controller, ui, simulator


def test_identity_validation_and_canonical_enclave_path() -> None:
    identity = EnclaveIdentity("lab_a", "simulator", "sim-west-2")

    assert identity.enclave == "/elesim/lab_a/simulator/sim_west_2"
    assert identity.relative_enclave == Path("elesim/lab_a/simulator/sim_west_2")

    with pytest.raises(ValueError, match="system_id"):
        EnclaveIdentity("../lab", "simulator", "sim-west")
...
def test_generation_uses_ros2_cli_and_role_policy(
    ...
    policy = tmp_path / "controller.policy.xml"
    ...
    identity = generation.create_enclave(
        "controller", "controller-main", policy=policy
    )
...
def test_real_sros2_file_links_are_materialized_before_distribution(
    ...
    identity = generation.create_enclave("controller", "controller-main")
    assert not any(path.is_symlink() for path in generation.keystore.rglob("*"))
    ...
    assert (
        bundle.path
        / "roles/controller/keystore/enclaves"
        / identity.relative_enclave
        / "governance.p7s"
...
def test_host_bundle_contains_public_and_only_assigned_enclaves(
    ...
    authority, generation, controller, ui, simulator = _issued_generation(
        tmp_path, fake_security_runner
    )

    staged = generation.stage_host_bundle("operator-laptop", (controller, ui))
    generation.stage_host_bundle("compute-server", (simulator,))
    generation.publish()
    ...
    assert exported.manifest.enclaves == tuple(sorted((controller, ui)))
    assert (exported.path / "keystore/public/identity_ca.cert.pem").is_file()
    assert (
        exported.path / "keystore/enclaves" / controller.relative_enclave / "key.pem"
    ).is_file()
    assert (
    ...
    assert (
        exported.path
        / "roles/controller/keystore/public/identity_ca.cert.pem"
    ).is_file()
    assert not (
        exported.path
        / "keystore/enclaves"
        / simulator.relative_enclave
    ).exists()
    assert not (exported.path / "keystore/private").exists()
    assert not (exported.path / "roles/controller/keystore/private").exists()
...
def test_manifest_detects_tampering_and_unlisted_files(
    ...
    _, generation, controller, _, _ = _issued_generation(
        tmp_path, fake_security_runner
    )
    generation.stage_host_bundle("operator-laptop", (controller,))
    generation.publish()
    bundle = generation.export_host_bundle(
        "operator-laptop", tmp_path / "exported/operator-laptop"
    ).path
    certificate = (
        bundle / "keystore/enclaves" / controller.relative_enclave / "cert.pem"
    )
...
def test_enclave_cannot_be_distributed_to_two_hosts(
    ...
    with pytest.raises(SecurityAuthorityError, match="already assigned"):
        generation.stage_host_bundle("backup-laptop", (controller,))
...
def test_generation_activation_and_rollback_are_atomic_metadata(
    ...
    authority, first, controller, _, _ = _issued_generation(
        tmp_path, fake_security_runner, "g-0001"
    )
    first.stage_host_bundle("operator-laptop", (controller,))
    ...
    next_controller = second.create_enclave("controller", "controller-main")
    second.stage_host_bundle("operator-laptop", (next_controller,))
...
def test_export_is_no_overwrite_and_authority_is_bound_to_one_system(
    ...
    authority, generation, controller, _, _ = _issued_generation(
        tmp_path, fake_security_runner
    )
    generation.stage_host_bundle("operator-laptop", (controller,))
    generation.publish()
