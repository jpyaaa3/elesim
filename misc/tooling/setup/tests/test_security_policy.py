...
def test_controller_policy_limits_motion_and_rgbd_direction() -> None:
    rendered = render_role_policy(
        system_id="elesim",
        role="controller",
        endpoint_id=ENDPOINTS["controller"],
        endpoints=ENDPOINTS,
    )
    root = ET.fromstring(rendered)
    assert root.find(".//enclave").attrib["path"] == (
        "/elesim/elesim/controller/controller_main"
    )
    ...
    assert (
        f"/elesim/v5/peers/{peer_node_key('robot-go2')}/*/motion"
        in published
    )
    assert (
        f"/elesim/v5/peers/{peer_node_key('sim-default')}/*/motion"
        in published
    )
    assert (
        f"/elesim/v5/peers/{peer_node_key('ui-main')}/*/motion"
        not in published
    )
    ...
    assert "/elesim/v5/*/get_parameters" in _service_permissions(
        rendered, "reply"
    )
...
def test_policy_carrier_patterns_match_real_dds_peer_prefixes() -> None:
    ...
    controller_prefix = _real_peer_prefix("controller-main", "controller")
    controller_pattern = controller_prefix.rsplit("/", 1)[0] + "/*"
    robot_prefix = _real_peer_prefix("robot-go2", "robot")
    robot_pattern = robot_prefix.rsplit("/", 1)[0] + "/*"

    assert f"{controller_pattern}/control" in subscribed
    assert f"{controller_pattern}/motion" in subscribed
    assert f"{robot_pattern}/control" in published
    assert f"{robot_pattern}/motion" in published
    assert "/elesim/controller_main/rgbd/frame" not in subscribed
    assert "/elesim/robot_go2/rgbd/frame" in subscribed


@pytest.mark.parametrize("role", ("ui", "simulator", "robot"))
def test_non_controller_policies_cannot_publish_motion(role: str) -> None:
    rendered = render_role_policy(
        system_id="elesim",
        role=role,
