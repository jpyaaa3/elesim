...
from misc.setup import bootstrap as bootstrap_module
...
def _archive_payload(
    ...
    *,
    ...
    _archive(
        archive,
        {
            f"{root}/misc/tooling/setup/pyproject.toml": project,
            f"{root}/misc/tooling/setup/requirements.lock": b"",
            f"{root}/packages/protocol/pyproject.toml": b"[project]\n",
            f"{root}/misc/setup/bootstrap.py": b"# bootstrap\n",
            f"{root}/misc/setup/bootstrap-contract.json": json.dumps(
                {
                    "schema_version": 1,
                    "bootstrap_api": 1,
...
def _write_valid_snapshot(snapshot: Path, root_name: str = "elesim-main") -> None:
    ...
    (root / "misc/tooling/setup").mkdir(parents=True)
    (root / "misc/tooling/setup/pyproject.toml").write_text("[project]\n")
    (root / "misc/tooling/setup/requirements.lock").write_text("")
    (root / "packages/protocol").mkdir(parents=True)
    (root / "packages/protocol/pyproject.toml").write_text("[project]\n")
    (root / "misc/setup").mkdir(parents=True)
    (root / "misc/setup/bootstrap.py").write_text("# bootstrap\n")
    (root / "misc/setup/bootstrap-contract.json").write_text(
        json.dumps(
            {
                "schema_version": 1,
                "bootstrap_api": 1,
...
def test_download_source_uses_full_url_hash_and_ignores_legacy_cache(
    ...
    (legacy_root / "misc/tooling/setup").mkdir(parents=True)
    (legacy_root / "misc/tooling/setup/pyproject.toml").write_text("[project]\n")
...
def test_download_source_rejects_incomplete_same_revision_archive(
    ...
    _archive(
        invalid_path,
        {"elesim-invalid/misc/tooling/setup/pyproject.toml": b"[project]\n"},
        commit=commit,
    )
    opener = _URLSequence(
...
def test_download_source_refresh_repairs_existing_snapshot(
    ...
    url = "https://archives.example/elesim.tar.gz"
    first = download_source(url, tmp_path)
    contract = first / "misc/setup/bootstrap-contract.json"
    contract.unlink()
...
def test_download_source_retries_unconditionally_after_304_for_incomplete_snapshot(
    ...
    url = "https://archives.example/elesim.tar.gz"
    first = download_source(url, tmp_path)
    (first / "misc/tooling/setup/pyproject.toml").unlink()
...
def test_failed_shell_download_preserves_previous_bootstrap(tmp_path: Path) -> None:
    script = Path(__file__).resolve().parents[3] / "setup/bootstrap.sh"
...
def test_shell_forwards_custom_archive_without_exposing_it_in_docker_argv(
    ...
    script = Path(__file__).resolve().parents[3] / "setup/bootstrap.sh"
    bootstrap_source = Path(__file__).resolve().parents[3] / "setup/bootstrap.py"
    fake_bin = tmp_path / "bin"
    fake_bin.mkdir()
    curl = fake_bin / "curl"
    curl.write_text(
