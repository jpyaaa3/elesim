...
def _validate_source_snapshot(root: Path) -> None:
    required_files = (
        ...
        root / "misc/setup/bootstrap.py",
        root / "misc/setup/bootstrap-contract.json",
    )
...
def validate_bootstrap_contract(source_root: Path) -> dict[str, object]:
    path = source_root / "misc/setup/bootstrap-contract.json"
    try:
...
def validate_bootstrap_generation(
    ...
    *,
    ...
    if not executing_file.is_file():
        return
    archived = source_root / "misc/setup/bootstrap.py"
    if not archived.is_file():
        raise BootstrapError("downloaded archive is missing misc/setup/bootstrap.py")
    try:
...
def _setup_project_version(source_root: Path) -> str:
    try:
        project = (source_root / "misc/tooling/setup/pyproject.toml").read_text(
            encoding="utf-8"
        )
    except (OSError, UnicodeError):
        return "unknown"
