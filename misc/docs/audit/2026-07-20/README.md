...
1. `model/bundles/default` is not self-contained. Its URDFs resolve meshes from
   `misc/model/source`, and the release builder hides the defect by copying the whole
   model tree.
...
The detailed remediation order is maintained in `misc/docs/OPEN_ISSUES.md` and its
