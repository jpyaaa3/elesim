...
and imports no Controller, UI, Simulator, or Robot implementation.
...
General mode translates to state schema v7 and one of two backends:
...
any non-empty combination of Simulator, Controller, and UI; Robot is available
...
automation, repeat `--role` (for example `--role simulator --role ui`). The
...
from state schemas v1 through v6 keep it disabled so an upgrade does not begin
...
`sros2` enables DDS Security in enforce mode. State schema v7 distinguishes:
...
- `full`: assign Controller, UI, Simulator, and Robot exactly once across two to
  four active hosts. Robot is fixed to a native Jetson/systemd host.
- `simulation-only`: assign Controller, UI, and Simulator exactly once across
...
are read as `full` and normalized to schema v2 with an explicit mode.
...
State schema v7 keeps TURN endpoint URLs separate from relay ownership and
...
keystore/enclave. Schema-v5 SROS2 state migrates to `external` provisioning;
