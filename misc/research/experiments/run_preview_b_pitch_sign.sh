...
SOURCE_CONFIG="${SOURCE_CONFIG:-controller/config/default.yaml}"
...
_make_config() {
  local b_pitch="$1"
  python misc/research/experiments/config_overlay.py \
    --base "$SOURCE_CONFIG" \
    --output "$TMP_CONFIG" \
...
_run_batch() {
  ...
  PYTHONPATH=. python misc/research/experiments/run_walking_baseline_batch.py \
    --config "$TMP_CONFIG" \
    --gaze pitch_preview \
    --run-prefix "$prefix" \
    --preset "$PRESET" \
...
if [[ "${DRY_RUN:-0}" == "1" ]]; then
  ...
  echo "  analyze: PYTHONPATH=. python misc/research/analysis/analyze_preview_b_pitch_sign.py --log-dir $LOG_DIR"
  exit 0
fi
...
PYTHONPATH=. python misc/research/analysis/analyze_preview_b_pitch_sign.py --log-dir "$LOG_DIR"
