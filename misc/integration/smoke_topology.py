...
def _controller_process(barrier: Any, stop: Any, results: Any) -> None:
            ...
            while not stop.is_set():
                ...
                for message in peer.receive(timeout_ms=20):
                    if message.message_type == "target_selected":
                        selected = True
                        lease_id = message.lease_id
                        _report(results, "controller:selected")
                        ...
                        _report(results, "controller:operator")
                    elif message.message_type == "ack":
                        _report(results, "controller:ack")
    ...
    _run_child("controller", results, run)


def _simulator_process(barrier: Any, stop: Any, results: Any) -> None:
    def run() -> None:
        peer = PeerClient(
            EndpointDescriptor("simulator-smoke", "simulator", ()),
            settings=_settings(),
        )
        _wait_barrier(barrier)
        active_session = ""
            ...
            while not stop.is_set():
                ...
                for message in peer.receive(timeout_ms=20):
                    if message.message_type == "simulation_session_granted":
                        ...
                        _report(results, "simulator:session")
                        ...
                        _report(results, "simulator:command")
                        ...
                        _report(results, "simulator:webrtc")
    ...
    _run_child("simulator", results, run)
    ...
    def run() -> None:
        ...
        try:
            while not stop.is_set():
                ...
                if "controller-smoke" in endpoints and not operator_sent:
                    peer.send(
                        "operator_intent",
                        target_id="controller-smoke",
                        payload={
...
def main() -> int:
    ...
    processes = (
        ...
        context.Process(
            ...
            args=(barrier, stop, results),
            name="elesim-smoke-controller",
        ),
        context.Process(
            target=_simulator_process,
            args=(barrier, stop, results),
            name="elesim-smoke-simulator",
        ),
        context.Process(
            target=_ui_process,
            args=(barrier, stop, results),
