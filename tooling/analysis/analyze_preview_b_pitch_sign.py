...
from tooling.analysis.analyze_walking_metrics import summarize_run
...
def main() -> None:
    ...
    if not result["b_pitch_positive"]["count"] or not result["b_pitch_negative"]["count"]:
        print(
            "\nNo complete sweep yet. Run tools/run_preview_b_pitch_sign.sh after enabling gaze_preview_enable=true.",
            file=sys.stderr,
        )
