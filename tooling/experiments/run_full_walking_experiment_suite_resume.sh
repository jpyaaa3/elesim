...

{
  ...
  for rid in "${RUNS[@]}"; do
    python tools/analysis/analyze_walking_metrics.py "$rid" --log-dir "$LOG_DIR" --merged >/dev/null || true
  done

  echo "=== 4-way compare ==="
  python tools/analysis/analyze_walking_metrics.py --log-dir "$LOG_DIR" --compare \
