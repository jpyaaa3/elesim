...
COMMON=(
  --config deployments/controller/config/default.yaml
  --motion forward
  --preset neutral
  --max-duration 30
...

{
  ...
  for rid in "${RUNS[@]}"; do
    python tools/analysis/analyze_walking_metrics.py "$rid" --log-dir "$LOG_DIR" --merged "${ANALYZE_ARGS[@]}" >/dev/null || true
  done

  echo "=== 4-way compare (trial 005 representative; full trial) ==="
  python tools/analysis/analyze_walking_metrics.py --log-dir "$LOG_DIR" "${ANALYZE_ARGS[@]}" --compare \
