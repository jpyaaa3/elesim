...
from tooling.experiments.walking_baseline import _connect_service, _run_trial, _trial_run_id, _validate_gaze_config


def _host_reachable(config_path: str) -> bool:
    bundle = load_app_config(config_path)
    runtime = load_runtime_role_config(ROOT / "deployments/controller/config/runtime.yaml")
...
def _start_sim(config_path: str, run_id: str, log_path: Path) -> subprocess.Popen:
    ...
    print(f"[batch] starting simulator run_id={run_id!r} -> {log_path}")
    proc = subprocess.Popen(
        [sys.executable, "-m", "elesim_simulator.main", "--config", config_path],
        cwd=str(ROOT),
        env=env,
        stdout=fh,
        stderr=subprocess.STDOUT,
...
def main() -> None:
    ...
    ap.add_argument("--config", default="deployments/controller/config/default.yaml")
