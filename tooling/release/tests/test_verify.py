...
from tooling.release.verify import (
...
def test_wheel_boundary_accepts_only_the_owned_package(tmp_path: Path) -> None:
    wheel = _wheel(
        ...
        "elesim_controller-0.1.0.dist-info/METADATA",
    )
...
def test_wheel_environment_is_data_not_shell_code(tmp_path: Path) -> None:
    ...
    env_file.write_text(
        "PROTOCOL_WHEEL=elesim_protocol-0.1.0.whl\n"
        "APP_WHEEL=elesim_controller-0.1.0.whl\n",
        encoding="utf-8",
    )
