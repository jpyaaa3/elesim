...
Full stack (requires elesim-router + elesim-simulator + elesim-controller):
  ELESIM_PROFILE_PICK=1 python tools/profile_pick_e2e.py --phases look,ready

Compute-only (no host):
  ELESIM_PROFILE_PICK=1 python tools/profile_pick_e2e.py --compute-only --cprofile
"""
...
def main() -> int:
    ...
    ap.add_argument(
        "--config",
        default=str(ROOT / "deployments/controller/config/default.yaml"),
