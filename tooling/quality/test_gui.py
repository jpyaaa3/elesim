...
TEST_ROOTS = (
    ...
    ROOT / "deployments/router/tests",
    ROOT / "deployments/controller/tests",
    ROOT / "deployments/robot/tests",
    ROOT / "deployments/simulator/tests",
    ROOT / "deployments/ui/tests",
    ROOT / "tooling/model_builder/tests",
    ROOT / "tooling/analysis/tests",
    ROOT / "tooling/debug/tests",
    ROOT / "tooling/experiments/tests",
)
SOURCE_ROOTS = (
    ROOT / "packages/protocol/src",
    ROOT / "deployments/router/src",
    ROOT / "deployments/controller/src",
    ROOT / "deployments/robot/src",
    ROOT / "deployments/simulator/src",
    ROOT / "deployments/ui/src",
    ROOT / "tooling/model_builder/src",
)
WINDOW_W = 1280
WINDOW_H = 760
LEFT_W = 350.0
...
MAX_LOG_LINES = 4000
PYTEST_LOCATION_RE = re.compile(r"^(?:packages|deployments|tooling)/.+\.py:\d+(?::|$)")
...
def _existing(paths: Sequence[str]) -> tuple[str, ...]:
    ...
    for raw in paths:
        ...
        if raw.startswith("tests/"):
            path = "deployments/controller/tests/legacy/" + raw[len("tests/"):]
        if (ROOT / path).exists():
...
def _under(prefix: str) -> tuple[str, ...]:
    if prefix.startswith("tests/"):
        prefix = "deployments/controller/tests/legacy/" + prefix[len("tests/"):]
    ...
    TestCaseGroup(
        ...
        _existing(
            (
                ...
                "tests/scenarios/pick/test_02_controller_lease.py",
            )
        ),
...
TEST_GROUPS += (
    ...
    TestCaseGroup("라우터", _under("deployments/router/tests/"), "등록, discovery, lease와 role 기반 forwarding을 확인합니다."),
    TestCaseGroup("로봇", _under("deployments/robot/tests/"), "Jetson I/O 경계, q 명령과 local safety를 확인합니다."),
    TestCaseGroup("시뮬레이터", _under("deployments/simulator/tests/"), "Genesis adapter, model bundle과 virtual endpoint 계약을 확인합니다."),
    TestCaseGroup("UI", _under("deployments/ui/tests/"), "UI가 controller 구현 없이 operator protocol만 사용하는지 확인합니다."),
    TestCaseGroup("모델 빌더", _under("tooling/model_builder/tests/"), "blueprint와 URDF 생성 계약을 확인합니다."),
    TestCaseGroup("개발 도구", _under("tooling/"), "분석기, 로그뷰어와 실험 CLI의 순수 helper를 확인합니다."),
)
