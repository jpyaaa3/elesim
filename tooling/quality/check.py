...
CHECKS: tuple[Check, ...] = (
    ...
    Check(
        ...
        (PROTOCOL_SRC, "deployments/router/src"),
        description="Router registry, lease and forwarding behavior",
    ),
    Check(
        "robot",
        ("deployments/robot/tests",),
        (PROTOCOL_SRC, "deployments/robot/src"),
        description="Physical I/O boundary and local safety behavior",
    ),
    Check(
        "controller",
        ("deployments/controller/tests",),
        (PROTOCOL_SRC, "deployments/controller/src"),
        description="Vision, IK, gaze and Pick control behavior",
    ),
    Check(
        "simulator",
        ("deployments/simulator/tests",),
        (PROTOCOL_SRC, "deployments/simulator/src"),
        description="Genesis endpoint and virtual-robot behavior",
    ),
    Check(
        "ui",
        ("deployments/ui/tests",),
        (PROTOCOL_SRC, "deployments/ui/src"),
        description="Operator API and presentation state behavior",
    ),
    Check(
        "model-builder",
        ("tooling/model_builder/tests", "tooling/release/tests"),
        (PROTOCOL_SRC, "deployments/controller/src", "tooling/model_builder/src"),
        description="Blueprint, bundle and URDF generation",
    ),
    Check(
        "topology",
        ("integration/smoke_topology.py",),
        (PROTOCOL_SRC, "deployments/router/src"),
        module="script",
    ...
    Check(
        "debug-tools",
        ("tooling/debug/tests",),
        (PROTOCOL_SRC, "deployments/controller/src", "deployments/ui/src"),
        group="extended",
        description="Manual debugger support code",
    ),
    Check(
        "experiment-tools",
        ("tooling/experiments/tests",),
        (PROTOCOL_SRC, "deployments/controller/src", "tooling/model_builder/src"),
        group="extended",
