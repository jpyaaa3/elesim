...
from tooling.quality.check import CHECKS, PROTOCOL_SRC, python_path, select_checks
    ...
    def test_each_deployment_gets_only_its_own_source_root(self) -> None:
        ...
        for check in CHECKS:
            if check.name not in deployment_names:
                continue
            own_source = f"deployments/{check.name}/src"
            self.assertIn(PROTOCOL_SRC, check.python_paths)
            self.assertIn(own_source, check.python_paths)
            sibling_sources = {
                f"deployments/{name}/src" for name in deployment_names if name != check.name
            }
