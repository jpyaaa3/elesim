...
CASES: tuple[MutationCase, ...] = (
    MutationCase(
        ...
        source_root="packages/protocol/src",
    ...
    MutationCase(
        ...
        source_root="deployments/router/src",
        source_file="elesim_router/core.py",
        original="or request.lease_id != lease_id:",
        mutant="or False:",
        tests=("deployments/router/tests/legacy/test_router_server.py",),
        python_paths=("packages/protocol/src",),
    ),
    MutationCase(
        name="robot-stale-sequence",
        source_root="deployments/robot/src",
        source_file="elesim_robot/runtime.py",
        original="if envelope.seq <= self.last_seq:",
        mutant="if False:",
        tests=("deployments/robot/tests/legacy/scenarios/go2/test_12_agent_bridge.py",),
        python_paths=("packages/protocol/src",),
    ),
    MutationCase(
        ...
        mutant="if True:",
        tests=("deployments/robot/tests/test_runtime.py",),
        python_paths=("packages/protocol/src",),
    ),
    MutationCase(
        name="uv-control-direction",
        source_root="deployments/controller/src",
        source_file="elesim_controller/vision/visual_servoing/uv_jacobian.py",
        original="delta = -float(gain) * (pinv @ err)",
        mutant="delta = +float(gain) * (pinv @ err)",
        tests=("deployments/controller/tests/properties/test_uv_jacobian_properties.py",),
        python_paths=("packages/protocol/src",),
    ),
    MutationCase(
        name="lji-gain-direction",
        source_root="deployments/controller/src",
        source_file="elesim_controller/vision/visual_servoing/local_image_jacobian.py",
        original="s_stack = gains[active] * ordered_s[active]",
        mutant="s_stack = ordered_s[active] / gains[active]",
        tests=("deployments/controller/tests/properties/test_lji_properties.py",),
        python_paths=("packages/protocol/src",),
    ),
    MutationCase(
        name="equal-sag-finite-input",
        source_root="deployments/controller/src",
        source_file="elesim_controller/vision/visual_servoing/equal_sag_probe.py",
        original="if not np.all(np.isfinite(drift)) or not np.all(np.isfinite(j)):",
        mutant="if False:",
        tests=("deployments/controller/tests/properties/test_equal_sag_properties.py",),
        python_paths=("packages/protocol/src",),
    ),
)
