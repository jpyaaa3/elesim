...
def _select_profile(
    ...
    *,
    ...
    """Resolve the optional in-file deployment profile.

    A unified application config keeps shared values at the root and puts
    machine/runtime differences under ``profiles``.  ``extends`` is resolved
    first so an installed overlay can select a profile from the source file.
    The selected profile is merged over the shared values and the metadata is
    removed before strict schema validation.
    """
