...
def _bbox_tracker_from_config(cfg: "PerceptionConfig") -> Any:
    from elesim_pilot.vision.perception.visual_tracker import BboxTracker, CsrtTrackerTuning  # type: ignore[import-not-found]
...
class _PerceptionCaptureLifecycle(_PerceptionCaptureStorage):
    ...
    def _run(self) -> None:
        ...
        try:
            from elesim_pilot.vision.perception.pipeline import (
            ...
            from elesim_pilot.vision.perception.detector import create_detector, load_detector_config
            from elesim_pilot.vision.perception.preview import (
                close_preview,
...
class _PerceptionDetectionActions(_PerceptionCaptureLifecycle):
    ...
    def _refine_detection_for_publish(
        ...
        *,
        ...
        if erode_px <= 0:
            return det
        _ensure_pick_place_path()
        from elesim_pilot.vision.perception.detection_utils import refine_detection_mask  # type: ignore[import-not-found]
    ...
    def _world_from_sim_frame(frame: Any, p_camera: np.ndarray) -> Optional[tuple[float, float, float]]:
        ...
        if origin is None or look is None or right is None:
            return None
        try:
            from elesim_pilot.vision.sim_camera.pose import camera_point_to_world_from_axes
        ...
        if mock_world is not None:
            ...
            if sim_world is not None:
                ...
                print(
                    ...
                    "restart elesim-sim after update (world coords will use endpoint FK until then)"
                )
...
class _PerceptionCameraPipelines(_PerceptionDetectionActions):
    ...
    def _run_camera_search_track(self, **kwargs: Any) -> None:
        ...
        _ensure_pick_place_path()
        from elesim_pilot.vision.perception.detection_utils import bbox_xyxy_area, detection_init_bbox  # type: ignore[import-not-found]
    ...
    def _run_camera_yolo_seg(self, **kwargs: Any) -> None:
        ...
        _ensure_pick_place_path()
        from elesim_pilot.vision.perception.detection_utils import (  # type: ignore[import-not-found]
            bbox_xyxy_area,
            detection_center_pixel,
            detection_init_bbox,
            detection_mask_coast_aligned,
        )
        from elesim_pilot.vision.perception.visual_tracker import detection_from_bbox  # type: ignore[import-not-found]
...
class PerceptionCapture(_PerceptionMockPipelines):
    ...
    def _run_mock_search_track(self, **kwargs: Any) -> None:
        ...
        _ensure_pick_place_path()
        from elesim_pilot.vision.perception.detection_utils import detection_init_bbox  # type: ignore[import-not-found]
