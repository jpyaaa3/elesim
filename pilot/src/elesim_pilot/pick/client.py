"""Command port used by pilot workflows.

``PilotConnection`` owns DDS transport and attaches its thread-safe
...
from elesim_pilot.remote_state import RemoteState
...
class ControlClient:
    ...
    def _send(self, message: Mapping[str, Any], *, force: bool = False) -> None:
        with self._lock:
            ...
            sender = self._sender
        if sender is None:
            self._state.accept_error("pilot transport is not started")
            return
