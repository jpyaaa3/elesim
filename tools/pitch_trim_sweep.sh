...
echo ""
echo "Cases (edit config.ini mpc_pitch_trim_* then restart sim.py each time):"
...
echo "  python sim.py --config config.ini"
echo "  python tools/walking_baseline.py --run-id \$ELESIM_RUN_ID \\"
echo "    --preset bent_upward --motion backward --duration 15 --vx -0.35 --gaze off"
echo ""
echo "Compare:"
echo "  python tools/analyze_walking_metrics.py --evaluate-trim exp_trim_baseline_001 exp_trim_<case>_001"
