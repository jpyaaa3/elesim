#!/usr/bin/env python3
"""Walking baseline runner — ControlService only (host+sim must be pre-started)."""
...
def _connect_service(config_path: str) -> ControlService:
    ...
    client = ControlClient(
        endpoint=bundle.sim_config.host_ctrl_port,
        cfg=bundle.mapping_config,
    )
    host = client.refresh_state()
    if not host.connected:
        raise SystemExit("host not reachable — start host.py first")
...
def main() -> None:
    ...
    if args.headless_hint:
        print("[hint] set enable_viewer=false in configs/config.yaml for headless sim")

    if args.print_trim_sweep_checklist:
        print("Pitch-trim sweep (sim restart required per case):")
        print("  1. Edit mpc_pitch_trim_* in configs/config.yaml")
        print("  2. export ELESIM_RUN_ID=exp_trim_<case>_001")
        print("  3. restart sim.py with ELESIM_WALKING_METRICS=1")
    ...
    for trial in range(1, repeat + 1):
        ...
        if env_rid and env_rid != run_id:
            print(
                f"[baseline] trial {trial}/{repeat}: set ELESIM_RUN_ID={run_id!r} and restart sim.py, then re-run with "
