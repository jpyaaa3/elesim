...
`tools/` is a top-level operator workspace, not runtime code.  It contains
commands used to inspect, reproduce, run, and evaluate the system.  Reusable
behavior belongs in `engine/`; process composition belongs in `apps/`.
...
The former flat paths remain as compatibility launchers.  New code and new
documentation should use the directory above, for example:

```bash
python3 tools/quality/test_gui.py
python3 tools/analysis/analyze_walking_metrics.py --help
python3 tools/experiments/walking_baseline.py --help
```
