...
MAX_LOG_LINES = 4000
PYTEST_LOCATION_RE = re.compile(r"^tests/.+\.py:\d+(?::|$)")
    ...
    TestCaseGroup(
        ...
        _existing(
            (
                ...
                "tests/scenarios/pick/test_02_host_sources.py",
            )
        ),
        "움직이기 전 사전 점검입니다. 실제 적용 설정, 컨트롤러 소유권, host가 받아들이는 명령 source를 확인합니다.",
    ),
    TestCaseGroup(
        "10 룩",
        _existing(
...
class TestGui:
    ...
    def _command_for(self, group: TestCaseGroup) -> list[str]:
        ...
        if self.runner == "docker":
            joined = " ".join(paths)
            inner = f"cd /home/dev/ws/elesim && python3 -m pytest {joined}"
            return [
                "docker",
                "compose",
                "-f",
    ...
    def run(self) -> None:
        if glfw is None or imgui is None or GlfwRenderer is None:
            raise SystemExit(
                "테스트 GUI에는 glfw와 imgui가 필요합니다. ctrl.py와 같은 GUI 의존성을 설치하세요."
            )
        if not glfw.init():
