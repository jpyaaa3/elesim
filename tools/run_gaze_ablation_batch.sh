...
COMMON=(--config config.ini --motion forward --preset neutral
...
for rid in "${RUNS[@]}"; do
  python tools/analyze_walking_metrics.py "$rid" --log-dir logs/walking_baseline --merged >/dev/null
done

echo "=== 3-way compare (trial 001) ==="
python tools/analyze_walking_metrics.py --log-dir logs/walking_baseline --compare \
...
python tools/analyze_walking_metrics.py --log-dir logs/walking_baseline --compare "${RUNS[@]}" \
