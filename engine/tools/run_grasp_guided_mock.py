...
Requires host.py + sim.py + control panel config with perception mode=mock.

Example:
  python engine/tools/run_grasp_guided_mock.py --object 0.5 0.0 1.2
  python engine/tools/run_grasp_guided_mock.py --phases aim,grasp
"""
...
def main() -> int:
    ...
    if str(bundle.perception_config.mode).strip().lower() != "mock":
        print(
            "[mock-grasp] warning: [perception] mode is not 'mock' in config.ini — "
    ...
    try:
        ...
        if host_state is None or not host_state.connected:
            print("[mock-grasp] host not connected — start host.py and sim.py first")
            return 1
