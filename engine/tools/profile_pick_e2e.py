...
Full stack (requires host.py + sim.py running):
  ELESIM_PROFILE_PICK=1 python engine/tools/profile_pick_e2e.py --phases look,ready

Compute-only (no host):
  ELESIM_PROFILE_PICK=1 python engine/tools/profile_pick_e2e.py --compute-only --cprofile
"""
    ...
    *,
    ...
    try:
        ...
        if host_state is None or not host_state.connected:
            print("[profile] host not connected — start host.py and sim.py first")
            return 1
