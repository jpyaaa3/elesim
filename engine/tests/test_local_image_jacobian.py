...
class TestJacobianEstimate(unittest.TestCase):
    ...
    def test_seed_fallback_when_measured_j_ill_conditioned(self) -> None:
        est = ImageJacobianEstimator3D(
            window_size=8,
            seed_j=default_j_lji_seed(center_u_gain=0.1, center_v_gain=0.1),
            min_measured_samples=4,
        ...
        self.assertGreaterEqual(rank, 3)
        self.assertLess(float(cond), 100.0)
        self.assertTrue(
            est.is_usable(min_samples=4, condition_max=100.0, min_rank=3)
        )
        self.assertTrue(np.allclose(j[2, :], 0.0))
...
class TestComputeDq3D(unittest.TestCase):
    ...
    def test_v_not_nulled_when_z_seg_coupling_matches(self) -> None:
        """Stacked solve keeps v seg correction when FK z row couples both segs."""
        j = default_j_lji_seed(
            center_u_gain=0.1,
            center_v_gain=0.1,
            command_direction=(-1, -1, 1, -1),
    ...
    def test_stacked_can_retract_linear_when_fk_row_says_so(self) -> None:
        """No mode switch: FK z row may prefer retract + bend over extend."""
        j = default_j_lji_seed(
            center_u_gain=12.0,
            center_v_gain=12.0,
            command_direction=(-1, -1, 1, -1),
...
class TestServo3D(unittest.TestCase):
    def test_servo_returns_dq(self) -> None:
        est = ImageJacobianEstimator3D(
            window_size=8,
            seed_j=default_j_lji_seed(center_u_gain=0.1, center_v_gain=0.1),
        )
        ...
        dq, _, _, rank, _, avail = servo.compute_dq(
            [0.05, -0.02, 0.15],
            z_row=z_row,
        )
