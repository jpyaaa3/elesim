    ...
    for service_name in ("tools", "runtime-tools"):
        ...
        if isinstance(tools, dict) and isinstance(tools.get("environment"), dict):
            tools["environment"].update(
                installer._dds_environment(state.runtime_roles[0], enclave_override=True)
            )
            # The doctor borrows an assigned role identity and its XML. A
            # topology can stop using the first role in the installed inventory.
            selected = state.runtime_roles[0]
            replacements = {}
            for role in state.roles:
                root = app_directory(state, role) / "config"
                selected_root = app_directory(state, selected) / "config"
                replacements[f"{root}/cyclonedds.xml:/opt/elesim/config/cyclonedds.xml:ro"] = (
                    f"{selected_root}/cyclonedds.xml:/opt/elesim/config/cyclonedds.xml:ro"
                )
                replacements[f"{root}:{root}:ro"] = f"{selected_root}:{selected_root}:ro"
                key = app_keystore_path(state, role)
                selected_key = app_keystore_path(state, selected)
                replacements[f"{key}:{key}:ro"] = f"{selected_key}:{selected_key}:ro"
            tools["volumes"] = [replacements.get(volume, volume) for volume in tools.get("volumes", [])]
...
class ContainerInstaller:
    ...
    def _write_tools_context(self) -> None:
        ...
        _reset_generated_context(context)
        source = root / "payload/runtime/docker/setup"
        _copy_source_tree(source, context)
    ...
    def _tools_service(self) -> dict[str, object]:
        context = self._build_root / "tools"
        dds_config = (
            app_directory(self.state, self.state.runtime_roles[0]) / "config" / "cyclonedds.xml"
        )
        volumes = [
    ...
    def _runtime_tools_service(self) -> dict[str, object]:
        ...
        role = self.state.runtime_roles[0]
    ...
    def _write_wrappers(self, refresh: OwnershipRefresh | None = None) -> None:
        ...
        write_executable(
            ...
            + (
                ...
                "      scoped_compose_after_action=0\n"
                "      scoped_compose_skip_value=0\n"
                "      scoped_compose_options=1\n"
                "      for scoped_compose_arg in \"$@\"; do\n"
                "        if (( ! scoped_compose_after_action )); then\n"
                "          [[ $scoped_compose_arg != \"$scoped_compose_action\" ]] || scoped_compose_after_action=1\n"
                "          continue\n"
                "        fi\n"
                "        if (( scoped_compose_skip_value )); then scoped_compose_skip_value=0; continue; fi\n"
                "        if (( scoped_compose_options )); then\n"
                "          case $scoped_compose_arg in\n"
                "            --) scoped_compose_options=0; continue ;;\n"
                "            -e|--env|--env-from-file|--entrypoint|--name|-p|--publish|--pull|-u|--user|-v|--volume|-w|--workdir|--index|-l|--label|--cap-add|--cap-drop) scoped_compose_skip_value=1; continue ;;\n"
                "            --*=*|-e?*|-p?*|-u?*|-v?*|-w?*|-l?*) continue ;;\n"
                "            --rm|-T|--no-TTY|--no-deps|--build|-d|--detach|-i|--interactive|--privileged|--service-ports|--use-aliases|-q|--quiet|--quiet-build|--quiet-pull) continue ;;\n"
                "            -*) printf '%s\\n' 'Unsupported scoped Compose run/exec option.' >&2; exit 64 ;;\n"
                "          esac\n"
                "        fi\n"
                # Only SERVICE is a lifecycle target. COMMAND and its arguments
                # may legitimately contain role names (doctor --instance-role).
...
def _runtime_up_wrapper(
    *,
    ...
    compose_argument_validation = (
        ...
        + "assigned_roles=$(python3 -c "
        + shlex.quote(
            'import json,sys; s=json.load(open(sys.argv[1], encoding="utf-8")); '
            'r=s.get("assigned_roles"); '
            'r=sys.argv[2:] if r is None else r; '
            'assert isinstance(r,list) and r and all(isinstance(v,str) and v in sys.argv[2:] for v in r) and len(r)==len(set(r)), "invalid assigned_roles"; '
            'print(" ".join(r))'
        )
        + " " + shlex.quote(str(state_path)) + ' "${runtime_roles[@]}") || exit 78\n'
        + 'read -r -a runtime_roles <<< "$assigned_roles"\n'
        "if (( ${#compose_args[@]} == 0 )); then\n"
        "  compose_args=(\"${runtime_roles[@]}\")\n"
        "else\n"
        "  for argument in \"${compose_args[@]}\"; do\n"
        '    if [[ $argument != coturn && " $assigned_roles " != *" $argument "* ]]; then\n'
        "      printf 'Role is not assigned to this host: %s\\n' \"$argument\" >&2; exit 64\n"
        "    fi\n"
        "    case $argument in\n"
