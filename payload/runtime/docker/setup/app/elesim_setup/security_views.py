"""Install stable runtime app views containing exactly one SROS2 role enclave."""
...
from .configuration import dds_enclave, app_keystore_path
from .state import InstallState


def prepare_app_keystore_views(state: InstallState) -> dict[str, Path]:
    """Create app roots and, when available, copy one enclave into each.
    ...
    sibling enclave directories are never copied into an app view.
    """
    ...
    _private_directory(apps_root)
    destinations = {
        role: app_keystore_path(state, role) for role in state.roles
    }
    ...
    *,
    ...
    try:
        for role, destination in destinations.items():
            ...
            for name in ("public", "enclaves"):
                current = destination / name
                replacement = prepared / role / name
...
__all__ = ["prepare_app_keystore_views"]
