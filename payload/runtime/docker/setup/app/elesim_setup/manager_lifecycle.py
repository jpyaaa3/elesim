...
def compose_owner_guard(
    ...
    *,
    ...
    return (
        ...
        "    printf 'EleSim fixed container name conflict: %s\\n' \"$container\" >&2\n"
        "    printf '  existing owner: project=%s compose=%s\\n' "
        "\"$actual_project\" \"$actual_compose\" >&2\n"
        "    printf '  current install: project=%s compose=%s\\n' "
        "\"$expected_project\" \"$expected_compose\" >&2\n"
        "    printf \"Stop and remove it with the existing install's elesim-down, then retry.\\n\" >&2\n"
        "    exit 73\n"
        "  fi\n"
        "done\n"
    )
...
def manager_lifecycle_fragment(
    ...
    *,
    ...
    return (
        ...
        f"    printf '%s is already running. Stop the existing connection manager or use another terminal.\\n' {display_name} >&2\n"
        "    exit 73\n"
        "  fi\n"
        "  if [[ $manager_owner != "
        + quoted_uuid
        + " || -z $manager_invocation || -z $manager_id ]]; then\n"
        f"    printf 'Existing %s belongs to another install or invocation. Stop the existing connection manager or use another terminal.\\n' {display_name} >&2\n"
        "    exit 73\n"
        "  fi\n"
...
def host_helper_fragment(
    *,
    ...
    return (
        ...
        "    printf 'EleSim host helper exited before startup.\\n' >&2\n"
        "    exit 2\n"
        "  fi\n"
        "  sleep 0.05\n"
        "done\n"
        "if [[ ! -S $host_helper_socket ]]; then\n"
        "  printf 'Timed out waiting for the EleSim host helper socket.\\n' >&2\n"
        "  exit 2\n"
        "fi\n"
        "manager_options+=(\n"
