...
LEGACY_TOPOLOGY_MODES = frozenset({"full", "simulation-only"})
...
class ConnectionTopology:
    ...
    def validate(self) -> "ConnectionTopology":
        ...
        if not 1 <= len(self.hosts) <= len(ROLES):
            raise ValueError("a managed topology requires 1..4 active hosts")
        for host in self.hosts:
            host.validate()
    ...
    def discovery_peers(self, host_id: str) -> tuple[str, ...]:
        ...
        # local locator for a distributed topology with co-located roles leaves a
    ...
    def from_dict(cls, raw: Mapping[str, Any]) -> "ConnectionTopology":
        ...
        if incoming_version == LEGACY_CONNECTION_SCHEMA_VERSION:
            ...
            legacy_topology_mode = _required_string(
                values["topology_mode"], name="topology_mode"
            )
            if legacy_topology_mode not in LEGACY_TOPOLOGY_MODES:
                raise ValueError(
