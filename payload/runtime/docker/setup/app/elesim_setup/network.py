...
def _configure_from_args(state: InstallState, args: argparse.Namespace) -> InstallState:
    ...
    return replace(
        state, network=network, dds=dds, turn=turn,
        assigned_roles=(
            state.assigned_roles if args.assigned_role is None
            else tuple(args.assigned_role)
        ),
    ).require_runnable_dds()
...
def _parser() -> argparse.ArgumentParser:
    ...
    configure.add_argument("--assigned-role", action="append", choices=("pilot", "sim", "ui", "robot"))
...
def _apply_configuration_transaction(
    ...
    for role in updated.runtime_roles:
