...
class WizardApplication:
    ...
    def context(self) -> dict[str, object]:
        return {
            ...
            "defaults": {
                ...
                "developer_workspace": str(self.invocation_dir),
    ...
    def list_directories(self, requested: Path) -> dict[str, object]:
    ...
    def validate_request(self, payload: Mapping[str, Any]) -> dict[str, object]:
        ...
        return {
            ...
            "developer_attachment": request.developer_attachment.enabled,
            "developer_workspace": (
                str(request.developer_attachment.workspace_path)
                if request.developer_attachment.workspace_path is not None
                else ""
            ),
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if parsed.path.startswith("/api/"):
            ...
            if parsed.path == "/api/directories":
                ...
                self._call(
                    lambda: self.server.application.list_directories(Path(value))
                )
                return
