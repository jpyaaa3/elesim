...
_MODEL_CONFIGS = {
    "yolo": "config/perception/detector.yolo.example.json",
    "hsv": "config/perception/detector.sim_hsv.json",
    "green_hsv": "config/perception/detector.real_green_hsv.json",
}
...
def browse_detector_config_path(initial_path: str) -> str | None:
    try:
        ...
        default_dir = config_root / "perception"
        ...
        try:
            relative = selected_path.resolve().relative_to(config_root.resolve())
            return str(Path("config") / relative)
        except ValueError:
