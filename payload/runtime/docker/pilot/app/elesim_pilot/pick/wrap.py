...
class WrapGraspConfig:
    ...
    #: Exported policies are durable runtime data, not application config.
    #: General installs mount the shared data tree at `/opt/elesim/data`.
    policy_path: str = "data/policies/wrap-grasp/policy.pt"
    manifest_path: str = "data/policies/wrap-grasp/interface.json"
    #: Fallbacks tried when the paths above are missing, in order.
    search_roots: tuple[str, ...] = (
        "/opt/elesim", "payload", ".", "deploy",
    )
    ...
    #: reports "waypoint was not reached" and stops the attempt on the first slow move.
...
class WrapGraspRunner:
    ...
    def __init__(
        ...
        *,
        ...
        from .wrap_policy import DeployedPolicy, LiftScript

        self.cfg = cfg
        policy, manifest = self._locate(cfg)
        # Validate against the service's effective mapping, when supplied.
        # This matters for calibrated hardware: the protocol defaults are not
        # necessarily the values used by this Pilot instance.
        self.policy = DeployedPolicy(policy, manifest, mapping=mapping)
    ...
    def _locate(cfg: WrapGraspConfig) -> tuple[Path, Path]:
        ...
        raise FileNotFoundError(
            f"Could not find the exported policy. Searched: {looked}. "
            f"Place policy.pt and interface.json generated by elesim_sim.rl.export "
            f"under payload/data/policies/wrap-grasp/ or the installed "
            f"data/policies/wrap-grasp/ directory."
        )
    ...
    def _load_proxy(self) -> Optional[Sequence[float]]:
        ...
        do exist.
        """
        if not self.policy.iface.expects_load:
            return None
    ...
    def run(self) -> WrapGraspOutcome:
        ...
        for _ in range(int(limit)):
            ...
            if over:
                ...
                out.reason = "load limit"
                _say(f"Stopped: load limit {over} (step {out.steps})")
                return out
            joints = list(self._read_joints())
            if len(joints) != 4:
                out.reason = f"expected 4 joint estimates, received {len(joints)}"
                return out
            ...
            if not reached:
                out.reason = "waypoint was not reached"
                _say(f"Stopped: {out.reason} (step {out.steps})")
                return out
            ...
            if lift:
                ...
                if not (curled or loaded_now):
                    ...
                    _say(f"Lift allowed: curl={curled} load={loaded_now}")

            if lift:
                out.lift_requested = True
                self._run_lift(waypoint, out)
                out.reason = "lift" if out.lift_completed else "lift aborted"
                return out

        out.reason = "step budget exhausted"
        return out
    ...
    def _run_lift(self, waypoint: Sequence[float], out: WrapGraspOutcome) -> bool:
        ...
        if self._command_roll is None:
            _LOG.warning("Lift was requested, but no roll command path is available")
            return False
        substep = 0
        ...
        for _ in range(guard):
            ...
            if over:
                out.overload_ma = over
                _say(f"Lift aborted: load limit {over}")
                return False
            ...
            _say("Lift script did not finish within its step budget")
            return False
...
class WrapActions:
    ...
    def wrap_grasp_config(self) -> WrapGraspConfig:
        ...
        if path is not None and path.is_file():
            try:
                ...
                _say(f"Loaded configuration: {path}")
            except Exception as exc:        # a bad file must not be silent
                _say(f"Could not read {path}; using defaults: {exc}")
                cfg = WrapGraspConfig()
        self._wrap_cfg = cfg
        return cfg
    ...
    def _run_wrap_grasp(self) -> None:
        try:
            ...
            _LOG.exception("Wrap grasp failed")
            self._wrap_result = f"Error: {exc}"
        else:
            self._wrap_result = wrap_summary(outcome)
            _say(f"Result: {self._wrap_result}")
    ...
    def _wrap_grasp_attempt(self) -> WrapGraspOutcome:
        ...
        runner = WrapGraspRunner(
            ...
            currents=currents,
            loaded=loaded,
            mapping=self.control_mapping() if hasattr(self, "control_mapping") else None,
        )
        outcome = runner.run()
        _LOG.info(
            "Wrap grasp: %d steps, lift requested %s, completed %s (%s)",
...
def wrap_summary(outcome: Optional[WrapGraspOutcome]) -> str:
    ...
    if getattr(outcome, "lift_completed", False):
        ...
        return f"Lift executed · {steps} steps · roll {turned:.0f}° (retention unverified)"
    if getattr(outcome, "lift_requested", False):
        return f"Lift aborted · {steps} steps ({reason})"
    if str(reason) == "load limit":
        over = getattr(outcome, "overload_ma", {}) or {}
        return f"Aborted · {steps} steps (load limit {over})"
    held = int(getattr(outcome, "lift_requests_held", 0) or 0)
    if held:
        lo1, hi1, lo2, hi2 = getattr(outcome, "theta_seen", (0.0, 0.0, 0.0, 0.0))
        return (f"Incomplete · {steps} steps ({reason}, {held} lift requests held, "
                f"th1 {lo1:+.2f}~{hi1:+.2f} th2 {lo2:+.2f}~{hi2:+.2f})")
    return f"Incomplete · {steps} steps ({reason})"
