...
class AimMotionActions:
    def _pick_reach_model(
        ...
        from elesim_pilot.robot.arm.iklib.kinematics import _ReachModel
    ...
    def _pick_ee_axis_world(
        ...
        *,
        ...
        from elesim_pilot.robot.arm.iklib.kinematics import _forward_link_tf
    ...
    def _pick_extend_cartesian(
        ...
        """Advance grasp point ``distance_m`` along EE local -Z via ``elesim_pilot.robot.arm.ik.solve_then_align``."""
...
class AimWorkflowActions(AimCenteringActions):
    ...
    def start_aim(self) -> None:
        ...
        if self._pick_look_ready_pose_world_xyz is None or self._pick_look_object_world_xyz is None:
            self._set_pick_failure("run Look first")
            return
        ...
        if initial_ready is None:
            self._set_pick_failure("aim: cannot compute initial ready pose from look object")
            return
        ...
        def _worker() -> None:
            try:
                ...
                if not self._wait_for_track_lock(
                    ...
                    self._set_pick_failure("aim acquire timeout")
                    return
                ...
                for it in range(max_iters):
                    ...
                    if obs is None:
                        ...
                        if stale_count >= 3:
                            if self._pick_apply_fov_search_step(reason="aim_observation_lost"):
                                ...
                                time.sleep(0.05)
                                continue
                            self._set_pick_failure("aim observation lost | fov search exhausted")
                            return
                        time.sleep(0.05)
                        continue
                    stale_count = 0
                    ...
                    if conv.center_ok:
                        ...
                        if estimate is not None and bool(estimate.accepted):
                            ...
                            self._set_pick_failure(f"aim centered but equal sag rejected | {reason}")
                        return
                ...
                self._set_pick_failure("aim iteration limit")
            finally:
...
class AimActions(AimWorkflowActions):
    ...
    def start_pick_forward(self, *, distance_m: float = 0.05) -> None:
        ...
        if step_m <= 1e-9:
            self._set_pick_failure("invalid pick distance")
            return
        host_state = self.client.refresh_state()
        ...
        if tip is None or direction is None:
            self._set_pick_failure("missing actual_tip feedback; run sim/host feedback first")
            return
        ...
        if dnorm <= 1e-9:
            self._set_pick_failure("tcp direction is zero")
            return
        ...
        if any(k not in ctx for k in required):
            self._set_pick_failure("missing IK context")
            return
        ...
        def _worker() -> None:
            try:
                ...
                if result.success and result.q is not None:
                    ...
                    self._set_pick_failure("pick IK failed | " + str(result.reason))
            finally:
        ...
        def _worker() -> None:
            try:
                ...
                if not self._wait_for_track_lock(
                    ...
                    self._set_pick_failure("track acquire timeout")
                    return
                    ...
                    if obs is None:
                        ...
                        if stale_count >= 3:
                            if self._pick_apply_fov_search_step(reason="observation_lost"):
                                ...
                                time.sleep(0.05)
                                continue
                            self._set_pick_failure("observation lost | fov search exhausted")
                            return
                        time.sleep(0.05)
                        continue
                    stale_count = 0
                ...
                self._set_pick_failure("iteration limit")
            except Exception as exc:
                print(f"[Pick] failed: {exc}")
                self._set_pick_failure(str(exc))
            finally:
                self._pick_frozen_world_xyz = None
                self._pick_worker = None
