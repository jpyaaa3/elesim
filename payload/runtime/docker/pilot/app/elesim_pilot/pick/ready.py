...
class ReadySolveActions(ReadyGeometryActions):
            ...
            try:
                ...
                if bool(resolve_dir):
                    ...
                    if (not result.success) or result.q is None:
                        ...
                        self._set_pick_failure(f"{label} IK failed | {result.reason}")
                        return
    ...
    def _start_grasp_to_object(self, *, internal: bool = False) -> bool:
        ...
        if bool(pk.local_img_jacobian_enabled) and self._use_hardware and not self._host_native_lji_runtime():
            self._set_pick_failure("hardware grasp requires host-native LJI runtime")
        ...
        try:
            ...
            self._set_pick_failure("cannot compute grasp target from approach direction")
            return False
...
class LookActions(ReadySolveActions):
    ...
    def start_look(self) -> None:
        ...
        object_world = self._pick_latest_object_world() or self._pick_frozen_world()
        if object_world is None:
            self._set_pick_failure("no object world coordinate")
            return
        ...
        try:
            ...
            self._set_pick_failure(f"reach model failed: {exc}")
            return
        ...
        if auto_dir is None:
            self._set_pick_failure("cannot infer look seed direction")
            return
        ...
        def _worker() -> None:
            ...
            try:
                ...
                if fail_msg is not None:
                    ...
                    self._set_pick_failure(str(fail_msg))
                    return

                if q is None or target_arr is None or look_dir_used is None:
                    self._set_pick_failure("look: missing IK solution")
                    return
...
class ReadyActions(LookActions):
    """Public ready-pose action exposed to the pilot service."""
    ...
    def start_ready_pose(self) -> None:
        ...
        if use_corrected:
            ...
            if object_world is None:
                self._set_pick_failure("corrected ready missing centered object")
                return
            ...
            if target_world is None:
                self._set_pick_failure("cannot compute pre-grasp target")
                return

        if dir_tuple is None:
            self._set_pick_failure("cannot infer pre-grasp direction")
            return
