...
def render_role_policy(
    *,
    ...
    The current protocol-v6 runtime uses topics only. Generated-but-unwired ROS
...
def _validated_endpoints(
    ...
    if not active_roles or not active_roles.issubset(SROS2_ROLES):
        raise ValueError(
            "endpoints must be a non-empty subset of pilot, sim, ui and robot"
        )
