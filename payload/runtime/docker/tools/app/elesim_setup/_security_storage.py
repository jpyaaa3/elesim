...
def verify_bundle(root: Path) -> BundleArtifact:
    ...
    view_name = "apps" if manifest.schema_version >= 2 else "roles"
    for identity in manifest.enclaves:
        app_keystore = bundle / view_name / identity.role / "keystore"
        app_public = app_keystore / "public"
        app_enclave = (
            app_keystore / "enclaves" / identity.relative_enclave
        )
        for required in (app_public, app_enclave):
            if required.is_symlink() or not required.is_dir():
                raise SecurityAuthorityError(
                    f"bundle is missing app view: {required.relative_to(bundle)}/"
                )
        _require_matching_tree(public_root, app_public)
        _require_matching_tree(
            enclaves_root / identity.relative_enclave,
            app_enclave,
        )
        allowed_roots.extend((app_public.resolve(), app_enclave.resolve()))
...
def _require_matching_tree(source: Path, mirror: Path) -> None:
    ...
    if source_files != mirror_files:
        raise SecurityAuthorityError(
            f"app view does not mirror assigned keystore tree: {mirror}"
        )
