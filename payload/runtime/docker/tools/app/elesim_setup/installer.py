...
class Installer:
    ...
    def _validate_source(self) -> None:
        ...
        required.extend(
            (
                root / "payload/runtime/native/robot/app/pyproject.toml"
                if role == "robot"
                else root
    ...
    def _install_tools(self) -> None:
        ...
        self._pip(python, "install", "-r", str(root / "payload/runtime/docker/tools/app/requirements.lock"))
        self._pip(
            python,
            "install",
            "--force-reinstall",
    ...
    def _install_role(self, role: str) -> None:
        root = self.state.source_path
        runtime = (
            root / "payload/runtime/native/robot"
            if role == "robot"
            else root / "payload/runtime/docker" / role
        )
        source = runtime / "app"
        ...
        bind_installed_data_paths(
            target / "config",
            role,
            runtime_data_root=self.state.prefix_path / "data",
        )
        ...
        self._pip(python, "install", "-r", str(runtime / "requirements.lock"))
    ...
    def _install_ros_interfaces(self) -> None:
        ...
        command = (
            ...
            "--base-paths "
            f"{shlex.quote(str(root / 'payload/runtime/common/elesim_interfaces'))} "
            "--build-base "
    ...
    def _write_wrappers(self) -> None:
        ...
        if self.state.roles == ("robot",):
            role_root = app_directory(self.state, "robot")
            write_executable(
    ...
    def _log_robot_service_registration(self, services: Sequence[Path]) -> None:
        ...
        access_roots = (
            app_directory(self.state, "robot"),
            self.state.prefix_path / "ros",
    ...
    def _dds_environment(
        ...
        *,
        ...
        environment = {
            ...
            "CYCLONEDDS_URI": (
                "file://"
                + str(app_directory(self.state, config_role) / "config/cyclonedds.xml")
            ),
            "ELESIM_DDS_VENDOR_CONFIG": str(
                app_directory(self.state, config_role) / "config/cyclonedds.xml"
            ),
        }
            ...
            environment.update(
                {
                    ...
                    "ROS_SECURITY_KEYSTORE": str(
                        app_keystore_path(self.state, identity_role)
                    ),
    ...
    def _role_command(self, role: str) -> tuple[Path, tuple[str, ...]]:
        root = app_directory(self.state, role)
