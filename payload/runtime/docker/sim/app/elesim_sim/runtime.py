...
class SimScene(_SimSceneKinematicsMixin):
    ...
    def reset_sim_target(self, xyz: np.ndarray) -> bool:
        ...
        if not np.all(np.isfinite(pos)):
            raise ValueError("sim target reset position must be finite")
        try:
            target.set_pos(pos)
            target.set_quat(np.asarray([1.0, 0.0, 0.0, 0.0], dtype=float))
            target.zero_all_dofs_velocity()
        except Exception as exc:
        ...
        try:
            while True:
                ...
                if q_errmodel is not None:
                    ...
                    if bool(getattr(a.cfg, "perf_log_enable", False)) and (_prev is None or (
