...
Run::
    ...
    python -m elesim_sim.rl.watch_eval \\
        --host 147.46.175.23 \\
        --remote-run ~/elesim/var/rl/sim/wrap_grasp/stage2_srv_v5 \\
        --interval 100
"""
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument("--out", default=None,
                        help="local output directory (default: var/rl/sim/eval/<run>)")
    ...
    local_run = _REPO_ROOT / "var/rl/sim" / "wrap_grasp" / f"remote_{run_name}"
    out_dir = (
        Path(args.out) if args.out
        else _REPO_ROOT / "var/rl/sim" / "eval" / run_name
    )
