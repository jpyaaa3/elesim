...
from engine.config import HardwareConfig, PerceptionConfig, PickConfig, load_app_config
from engine.gaze.stabilizer import (
...
from engine.observability.tracing import configure_tracing, message_span, shutdown_tracing, span
...
class ControlHost:
    ...
    def __init__(
        ...
        *,
        ...
        self.sim_pub_addr = str(sim_pub_addr)
        self.sim_feedback_addr = str(sim_feedback_addr)
        ...
        self._lji_velocity_hold_max_s = max(
            float(self._lji_velocity_hold_s),
            float(
                getattr(hardware_cfg, "lji_velocity_hold_max_s", 1.20)
                if hardware_cfg is not None
                else 1.20
            ),
        )
    ...
    def _new_on_device_panel_state(self):
        from engine.pick import PanelState
    ...
    def _ensure_on_device_control_service(self):
        ...
        from engine.pick import ControlService
    ...
    def _reply(self, ident: bytes, msg: Dict[str, Any]) -> None:
        ...
        try:
            with message_span(
                "host.ctrl.reply",
                outgoing,
                endpoint=self.bind_addr,
                direction="send",
            ):
                self.sock.send_multipart([ident, proto.dumps_msg(outgoing)], flags=0)
        except Exception:
            pass
    ...
    def _broadcast(self, msg: Dict[str, Any]) -> None:
        ...
        with message_span(
            ...
            outgoing,
            endpoint=self.sim_pub_addr,
            direction="send",
        ):
            data = proto.dumps_msg(outgoing)
        dead: Set[bytes] = set()
        ...
        try:
            ...
            print(
                ...
                % (
                    ...
                    float(self._lji_velocity_hold_s),
                    float(self._lji_velocity_hold_max_s),
        ...
        *,
        ...
        if float(self._lji_velocity_last_cmd_s) > 0.0:
            command_interval_s = max(
                0.0,
                float(apply_start) - float(self._lji_velocity_last_cmd_s),
            )
        adaptive_hold_s = float(self._lji_velocity_hold_s)
        if command_interval_s > 1e-3:
            adaptive_hold_s = max(
                adaptive_hold_s,
                min(float(self._lji_velocity_hold_max_s), 1.10 * float(command_interval_s)),
            )
        hold_s = max(float(adaptive_hold_s), velocity_dt)
            ...
            if self.sim_feedback in events and events[self.sim_feedback] & zmq.POLLIN:
                while True:
                    ...
                    try:
                        ...
                        continue
                    with message_span(
                        "host.sim_feedback.receive",
                        msg,
                        endpoint=self.sim_feedback_addr,
                        direction="receive",
                    ):
...
def main() -> None:
    configure_tracing("elesim-host")
    try:
        with span("host.process.run"):
            _run()
    finally:
        shutdown_tracing()
